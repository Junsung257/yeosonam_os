...
> **HWP 업로드 상업조건 자동화 (2026-08-02, 운영 스키마 적용):** `admin/upload`의 텍스트·파일·배치와 수동·자동 검수큐 재처리 경로는 실제 랜드사가 없으면 저장 전에 차단하고, 커미션을 비워두면 중앙 엔진이 기본 9%를 자동 입력한다. 기본값은 `commission_rate_defaulted` 검토 경고로 남으며 실제 계약 근거가 아니고 정산 전에 명시값·검증 원장으로 덮어쓸 수 있다. 파일명의 다른 숫자나 과거 평균은 여전히 근거로 인정하지 않으며 공개 스냅샷·모바일 증명 게이트는 독립 검사한다. 검증된 계약은 비공개 `product_commercial_contracts` 원장에 한 번 등록하면 명시적 파일명/원문 marker로 이후 상품에 자동 적용한다. 같은 우선순위에서 서로 다른 조건이 매칭되면 자동 선택하지 않고 보류한다. 다운로드 HWP 39개·상품 71개는 콘텐츠·이미지/원문 분리 감사 증빙이 완료됐고, 실제 고객 공개는 각 상품의 전체 publication/mobile 게이트 통과 후에만 가능하다.
>
> **HWP 1건씩 등록 원문 묶음 (2026-07-31, 로컬 증빙):** 위 39개 HWP를 현재 카탈로그 분리 규칙으로 71개 상품 TXT로 만들고 기존 오프라인 감사 SHA-256과 71/71 대조했다. 생성 뒤 TXT를 다시 읽은 해시도 71/71 일치하며, 중복 상품 키와 누락 원문은 0건이다. 묶음은 `scratch/upload-inbox-batch-reports/2026-07-28T23-28-40-714Z/upload-one-by-one-inputs/`에 있다. 각 TXT는 한 번의 `/admin/upload` 입력에만 사용하고, 상품별 실제 랜드사만 확인하면 되며 커미션은 비워두어도 기본 9%가 자동 입력된다. 이 증빙은 원문 분리 완전성 증명이며 DB 저장·고객 모바일 공개 증명은 아니다.

> **공용 다중상품 가격표 안전화 (2026-07-31, 로컬 코드):** 한 가격표에 3박5일 상품 5개와 4박6일 상품 1개가 함께 있는 실제 나트랑 저장 원문에서 기존 재처리기가 상품 열과 숙박 구간을 3단계 등급표로 축약하던 결함을 수정했다. 최대 8개 가격 열, 명시적 3박/4박 구간, 상품 정체성별 열을 보존한다. 같은 날짜의 할인 우선은 가까운 `특가`/`SPOT` 근거와 모든 열의 동일 할인 차이가 동시에 있을 때만 허용하고, 그 외 충돌은 고객 공개 차단으로 남긴다. 집중 회귀 39/39, 최신 전체 상품등록 회귀 894/894, 실제 저장 원문 20/20, 운영 빌드(정적 페이지 384/384·postbuild 산출물 검증)가 통과했으며 기존 상품 데이터·승인·공개 상태는 변경하지 않았다.

> **상품 모바일 이미지 승인 SSOT (2026-07-31, 운영 스키마·13건 적용):** `destination_metadata`는 목적지 대표 사진 후보와 공급자·원본 파일·라이선스·저작자·승인 출처·품질 점수·검증 근거를 보관하며 RLS는 `photo_approved=true` 행만 공개 읽기를 허용한다. Wikimedia Commons 후보는 여소남 Storage로 복사한 뒤 실제 이미지 바이너리, 목적지 식별어, Commons 파일 페이지, 허용 라이선스, 저작자 표시가 모두 통과하고 점수 0.95 이상일 때만 `automated_evidence_gate`로 승인한다. 현재 후보 13건은 동일 게이트를 13/13 통과해 승인됐고, `/packages`와 `/lp` 히어로에는 현재 이미지 URL과 정확히 일치하는 저작자·공급자·라이선스 링크를 표시한다. `travel_packages.hero_image_url`, `lp_hero_image_url`, `approved_destination_media`는 승인된 사진을 모바일/A4 proof 전에 상품 revision에 동결한다. 검색 첫 사진 자동 적용과 `/logo.png` 공개 통과는 계속 금지한다.
...
| 2 | **travel_packages** | `id`, `title`, `destination`, `country`, `duration`, `nights`, `price`, `cost_price`, `raw_text`, `parsed_data`(JSONB), `itinerary`(TEXT[]), `inclusions`(TEXT[]), `confidence`, `status`, `category`, `price_tiers`(JSONB), `surcharges`(JSONB), `hero_image_url`, `lp_hero_image_url`, `approved_destination_media`(JSONB), `tenant_id`(FK), `land_operator_id`(FK), `seats_held/confirmed/ticketed`, `is_airtel` | 여행 패키지 마스터 |
| 2-A | **destination_metadata** | `destination`(PK), `tagline`, `hero_tagline`, `hero_image_url`, `hero_image_provider`, `hero_image_pexels_id`, `hero_image_source_page_url`, `hero_image_source_file_title`, `hero_image_license`, `hero_image_license_url`, `hero_photographer`, `photo_approved`, `photo_approved_at`, `photo_approval_source`, `photo_quality_score`, `photo_verification_evidence` | 플랫폼 공통 목적지 카피·대표 이미지 승인 SSOT. 공개 읽기는 승인 행만 허용 |
| 2-B | **product_commercial_contracts** | `id`, `land_operator_id`, `contract_label`, `commission_rate`, 명시적 marker 배열, 유효기간, 근거 URL/hash, `verified_at`, `auto_apply`, `active`, `priority` | 서비스 역할 전용 계약 원장. 랜드사·커미션 자동 적용의 유일한 근거이며 anon/authenticated 접근 금지 |
