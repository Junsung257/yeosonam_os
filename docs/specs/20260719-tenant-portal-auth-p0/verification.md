...
- Rollout SQL follow-up: the dedicated RLS contract passed — 1 file, 13 tests. It proves Phase A has no existing-table/function mutation and Phase C creates no authenticated RFQ bid/proposal policy.
...
- SQL-contract lint: `src/lib/tenant-portal-rls-contract.test.ts` passed ESLint with zero warnings/errors.
...
- Full type-check: follow-up `npm run type-check` passed (exit 0, 68.7s) after the RFQ checker released the coordinated single-checker slot. It was not rerun for the final rollout-only change because that change is limited to SQL, Markdown, and static SQL-contract assertions; no production TypeScript changed.
...
- [ ] Record and verify the RFQ follow-up commit covering cron service-role persistence and active-membership/active-tenant checks for tenant actor actions.
...
1. **Phase A — additive DB gate:** verify production base tables/policies, approve and apply only `20260719171000_tenant_portal_memberships_rls.sql`, then provision each portal user's exact `auth.users.id` and tenant ID. The migration creates only the new membership table/index/grants/own-row RLS and does not change existing functions, policies, grants, or existing-table RLS. Do not derive access from JWT `user_metadata` or contact email. Prove active, cross-tenant, suspended, and unmapped-user outcomes before continuing.
2. **Phase B — application deploy gate:** only after Phase-A membership provisioning evidence exists, deploy the membership-bound tenant portal code together with RFQ commits `03e16701`, `653eba11`, and the pending RFQ follow-up. Before deploy, prove every RFQ cron path uses the service-role repository and every tenant actor bid/proposal/message action checks active membership plus active tenant status before repository access. Validate platform-admin S2S, one tenant login, inventory/product mutation ownership, settlements, RFQ list/detail, and draft/cancelled UUID denial.
3. **Phase C — RLS hardening gate:** `phase-c-rls-hardening-proposal.sql` is a proposal outside executable migrations. Convert it into a newly versioned migration only after Phase B is healthy, route/cron service-role reads and writes are verified in production-like runtime, tenant actor active-membership checks are proven, and a human explicitly approves the broad-policy and Jarvis function privilege changes. All four RFQ tables remain direct-authenticated default-deny; Phase C must not add browser membership policies for RFQ bids/proposals. Never apply the proposal file directly.
