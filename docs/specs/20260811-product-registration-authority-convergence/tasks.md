...
- The first live legacy batch started 25/25 workflows and terminalized 25/25 without publication. It exposed and fixed shared-source identity selection; evidence-rich rows are now prioritized and ledger terminal state synchronizes transactionally. The remaining 965 rows stay a bounded shadow gate.
- TypeScript, lint, authority/registration contracts, the full 710-file/5,256-test suite, and production build pass after the latest changes. Live 40-source workflow corpus, the remaining 965-row shadow backfill, paid provider calls, and a representative deployed browser-proof cohort remain release gates.
