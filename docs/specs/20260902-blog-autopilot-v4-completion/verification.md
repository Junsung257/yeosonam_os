...
- Gitleaks 8.28.0 PR-range scan: PASS, 6 commits scanned with 0 leaks. Three deterministic runtime resource IDs misclassified by the generic API-key rule are pinned by exact commit/file/rule/line fingerprints in `.gitleaksignore`; no path-wide or rule-wide exclusion was added.
...
- Dispatcher/protected-release hardening: the generation dispatcher now reuses the same research, demand, contract, representative, and duplicate selector as operational readiness. The protected release polls the operational database after dispatch and fails unless an approved generation run, completed selected attempt, private unpublished creative, and zero indexing-outbox rows are all proven. This closes the earlier false-positive where event acceptance alone was reported as a successful shadow.
