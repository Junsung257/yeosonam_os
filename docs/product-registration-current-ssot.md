...
Evidence is multi-source. `evidence.rawTextHash` remains the legacy representative hash, but `evidence.sourceDocuments[]` must carry the distinct source records used by registration: `original_raw`, `parser_raw`, `document_raw`, and `analysis_normalized` when available. Evidence spans may point to any registered source document hash; a span hash that is neither the legacy hash nor one of `sourceDocuments[].rawTextHash` is invalid.
