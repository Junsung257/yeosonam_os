...
Every upload requires the actual land operator. If the commission is omitted, the centralized intake automatically stores the platform fallback `9%` and emits a review warning (`commission_rate_defaulted`) so the operator does not have to repeat the same entry for every HWP product. The fallback is not supplier-contract evidence and must be corrected before settlement when the real contract differs. Invalid explicit values still fail closed. A filename may supply commercial values only when it explicitly follows the verified `[랜드사_커미션%]` form and that value is verified to apply to the specific product; unrelated filename numbers, supplier codes, and historical averages are never evidence. The publication gate independently blocks missing or invalid metadata so a manual/legacy row cannot bypass intake validation.

Verified recurring terms still belong in the private `product_commercial_contracts` master. `/admin/commercial-contracts` records the real operator, commission rate, validity window, evidence URL/hash, and explicit filename/raw-text markers once. Upload intake may replace the `9%` fallback only from an active, in-date, evidence-backed row whose configured marker matches exactly. Operator-name matching is opt-in. Equal-priority matches with different operator/rate values fail closed as `COMMERCIAL_CONTRACT_AMBIGUOUS`. Shorthand such as `15T`, `10T`, `8T`, or `TL` is never interpreted as a rate or operator unless an administrator deliberately registers that exact marker with contract evidence.

Content readiness and registration readiness are different metrics. A source may have correct price, itinerary, customer copy, entities, and approved media while still being ineligible for `admin/upload` registration because the land operator is unresolved. Batch audits and operator checklists must report both metrics and generate one commercial-input row per product, because products inside the same HWP may have different contracted commissions. Registration scripts must reject multi-file or multi-product batch writes that would apply one CLI commercial value to every product. The `9%` fallback is stored only in internal commercial fields and never enters customer copy. Customer opening still requires each saved package to pass the full publication and mobile proof gates.

When a reviewed HWP batch is converted into one-product paste inputs, `scripts/generate-upload-one-by-one-input-pack.ts` must reconstruct products with the same catalog splitter used by the audit and compare every product text SHA-256 with the existing audit record. A missing source, duplicate `{sourceFile, productIndex}`, changed split count, short text, or hash mismatch fails the whole generation. Generated TXT files contain only the audited product text; the manifest keeps the product-specific land-operator field and leaves the commission column blank with an explicit `9%` fallback note so verified filename/contract markers are not overridden. A successful input-pack hash check proves source partition integrity only, not registration or customer-mobile readiness.
...
- An attraction identity `hold` must carry a non-empty supplier confirmation request. The one-by-one checklist must include that request in both the required actions and copyable supplier message, covering the missing official identity evidence such as canonical local-language name, same-place confirmation, address, map link, or storefront/sign photo. A held product with no request fails checklist generation.
...
- Flattened shared catalogs may contain five or more product-price columns and separate `3박`/`4박` row groups. The deterministic reader must select the price column from the product identity and select the row group from the source-backed stay duration; it must not collapse these tables into a generic three-grade matrix or stop before a later duration group. A same-date lower row may override a higher row only when an explicit nearby `특가`/`SPOT` marker exists and every product column shows the same positive discount. Any other same-date price disagreement remains in the IR so the deliverability gate blocks publication for source review.
...
- `/logo.png` and `images_public[].source = "brand_fallback"` are preview-only safeguards. They may prevent a broken layout while an image is missing, but they do not satisfy `public_image_missing`, customer-open approval, or the mobile `*_customer_ready_image` proof check.
- Customer-open approval requires at least one auditable, approved product thumbnail, attraction photo, or destination image. A live renderer must not silently substitute a different attraction photo after the candidate snapshot is frozen.
...
상품 고유 이미지와 검증된 관광지 사진이 없을 때는 `destination_metadata`의 **승인 완료 행**만 목적지 fallback으로 사용할 수 있다. 승인은 어드민 직접 검수(`owner_reviewed`) 또는 결정론적 자동 근거 게이트(`automated_evidence_gate`)만 허용한다. 자동 게이트는 실제 저장 이미지 바이너리, 목적지 식별어, 공급자 파일 페이지, 저작자·라이선스 완전성을 모두 증명하고 `photo_quality_score >= 0.950`이어야 한다. 등록 엔진은 상품 `destination`과 승인 행의 `destination`이 정확히 같은 경우에만 URL·저작자·원본 상세 페이지·승인 시각을 `travel_packages.approved_destination_media`에 복사하고, 같은 revision의 `hero_image_url`/`lp_hero_image_url`로 모바일/A4 proof를 수행한다. 검색 결과, 미승인 후보, alias 추측, 공개 렌더 시점의 실시간 대체는 금지한다. 목적지 사진이 이후 변경되어도 이미 증명된 public snapshot은 바뀌지 않으며, 새 사진을 쓰려면 상품 revision 갱신과 모바일/A4 재검증이 필요하다. 승인 목적지 이미지가 히어로로 쓰이면 `/packages`와 `/lp`는 현재 히어로 URL과 일치하는 공개 스냅샷의 저작자·공급자·라이선스 링크를 표시한다.

목적지 후보 검색은 공급자 중립 계약을 사용한다. Wikimedia Commons는 원본 파일 페이지, 저작자, 허용 라이선스와 라이선스 URL이 모두 확인된 JPEG/PNG/WebP 가로 이미지에 한하며, Pexels는 유효한 자산 ID와 원본 상세 페이지가 필요하다. 검색 결과는 반드시 여소남 `destination-photos` Storage로 복사하고 최초에는 `photo_approved=false`, `photo_approved_at=null`로 저장한다. 한글 목적지명은 Storage 경로에 직접 쓰지 않고 NFC 정규화 후 SHA-256 기반 ASCII 키를 사용한다. 후보 저장 자체는 승인이나 상품 연결이 아니다. 매일 `/api/cron/destination-media-approval`이 미승인 저장 후보를 재검사하며, 자동 근거 게이트를 통과하지 못한 행은 익명 API, 상품 revision, public snapshot 어디에도 노출하지 않는다.

어드민 승인 목록은 `active_destinations`만 기준으로 만들지 않고 `destination_metadata` 후보 행과 합집합으로 구성한다. 아직 상품이 없는 목적지도 미승인 후보가 있으면 반드시 보여야 한다. 미승인 목록 API와 검색·저장 API는 라우트 자체에서 관리자 권한을 다시 확인하고 `private, no-store`로 응답한다. 승인 버튼은 저장 이미지, 공급자, 원본 페이지, 저작자·권리자와 공급자별 필수 근거가 모두 있을 때만 활성화하며, 저작자·라이선스·원본 링크를 화면에 표시하고 확인창을 거친다. 표준 `successResponse`의 `data` 봉투를 UI가 정확히 해석해야 하며 검색 결과나 저장 URL을 최상위 필드로 추정하지 않는다.

`노쇼핑`, `쇼핑 일정 없음`, `쇼핑 0회` 같은 명시 조건은 일반적인 `쇼핑 일정이 포함될 수 있습니다` 고지로 바꾸지 않는다. 고객 고지는 `쇼핑 일정 없음`으로 일치시킨다. 같은 원문이나 일정에 실제 쇼핑센터·매장 방문이 함께 있으면 `shopping_evidence_conflict`로 공개를 차단하고 공급사 확인 전까지 어느 한쪽을 추정해서 고치지 않는다.

`노쇼핑 진행 시 추가금`, `노쇼핑시 추가금`, `노옵션 진행 시 추가요금`처럼 별도 비용으로 조건을 바꾸는 문구는 기본 상품의 `노쇼핑`·`노옵션` 근거가 아니다. 이런 조건부 변경 문구만 있는 상품은 고객 제목이나 기본 조건 배지에 `노쇼핑`·`노옵션`을 붙이지 않는다. 원래 상품 제목 등 독립된 기본 조건 근거가 따로 있으면 그 근거만 유지한다.
