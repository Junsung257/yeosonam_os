...
- Local verification passes the complete 878-file Vitest regression (6,551 passed and 7 conditional skips), zero-warning ESLint, the 8 GB TypeScript profile, the 8 GB production build, postbuild output verification, and pinned native/WASM `rhwp` tracing. A read-only 390x844 production baseline also confirms the current logo/reference-image, repeated-hotel-copy, and unknown-flight-time defects; it is not evidence for the undeployed candidate.
- This candidate was implemented in an isolated worktree and applied only to the isolated Supabase project `product-registration-v61-rehearsal`. Normal, identical-retry, malformed-price rollback, immutable-mutation, blocked-product, service-role fail-closed catalog, and durable four-product/shared-price checks pass there. The Clark baseline binds one exact source hash to four revisions, 54 scoped departures and 54 immutable SELLING-price lineage rows with no excluded-date leakage. It has not written production, deployed code, changed a public pointer, or enabled auto-publish. Authenticated candidate browser proof remains mandatory. Production remains `publication_freeze=true` and `PRODUCTION READY: NO` until that proof and an explicitly approved rollout complete.
