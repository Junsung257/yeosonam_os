...
### Automation readiness update (2026-08-12 evening)

- The frozen 40-file HWP corpus now extracts and normalizes 40/40 files and yields 66 product sections. Under the V6 terminal-state policy, 1 is verified, 52 are safely degraded, and 13 are blocked, so 53/66 (80.30%) can be automatically concluded without forcing unsafe facts into the customer payload.
...
- Licensed destination reference media can be added through Pexels only with URL, content hash, license, source page, photographer, attribution, and a customer disclaimer. It never represents a generic destination image as the actual product photo.
...
- Production remains `authority_mode=shadow`, `publication_freeze=true`, and backfill disabled until the updated branch is deployed. Existing 990 packages are inventory for shadow classification, not candidates for bulk customer exposure.
...
- The latest verification passes TypeScript, lint, authority/registration contracts, the full 710-file/5,252-test suite, and a production Next.js build with 389 static pages. The local build took about 13 minutes 25 seconds; when Supabase/blog build-time credentials are absent, sitemap generation returns no package/blog entries rather than falling back to mutable rows.
