...
- The latest verification passes TypeScript, lint, authority/registration contracts, the full 710-file/5,259-test suite, and a production Next.js build with 389 static pages. When Supabase/blog build-time credentials are absent, sitemap generation returns no package/blog entries rather than falling back to mutable rows.
