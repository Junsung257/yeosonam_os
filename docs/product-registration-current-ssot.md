...
## V162 customer publication authority rebuild (2026-08-29, implementation candidate)

- Customer publication now has one authority path: immutable current revision → source-price lineage → grounded customer copy V3 → verified documentary product media → exact mobile proof → customer-only publication request → atomic customer pointer → cache outbox/convergence → live-domain canary. `travel_packages.status`, `products.status`, and legacy `approved` values are compatibility or operating projections and are never customer-publication authority.
- Source prices are typed as `NET`, `SELLING`, or `REQUEST_ONLY`. NET prices become a customer selling price only through the persisted margin policy; supplier selling prices are preserved without reverse-inventing NET; inquiry-only rows never receive a numeric price. The V62 commit wrapper stores one immutable `departure_price_lineage` row per departure in the same revision transaction. It does not UPDATE append-only departure facts. The V61 exact-date override remains a compatibility projection and is also append-only/idempotent.
- Customer copy policy `product-registration-customer-copy-v3` requires a grounded score of at least 82, blocks internal/supplier language and generic sales filler, checks numeric/source/cross-product contamination, and limits the AI rewrite budget to 900 output tokens. Copy is allowed to persuade through source-backed specifics, not through invented facts.
- Candidate publication requires at least one exact product/revision media asset supplied by the operator or supplier with verified rights/attribution, `safe` content safety, and `verified` relevance. Destination stock, logos, reference-only assets, and unverified generated media cannot satisfy customer hero readiness.
- `/admin/packages` is a read-only legacy compatibility list. Operators request publication only from `/admin/product-registration`, where the current revision, future/priced dates, price lineage, copy policy/score, documentary media, mobile proof, pointer/snapshot, public-catalog membership, blockers, and next action are projected together. The browser does not write publication state directly.
- Public home, package list/detail, LP, destinations, search, sitemap, recommendation, marketing, review, print, and Jarvis egress consume the same published-only catalog contract. The catalog fails closed for expired inventory, stale/weak copy, unverified documentary media, missing generation lineage, or a non-current pointer.
- Customer publication is independent of B2B/partner channels. The workflow fences the expected customer pointer, persists immutable proof, publishes the customer pointer atomically, emits cache convergence work, verifies `/packages` and `/lp`, and hides the customer pointer on failed convergence/canary. It does not weaken the existing freeze or restore an older pointer without a separately authorized rollback command.
- Local verification passes the complete 878-file Vitest regression (6,551 passed and 7 conditional skips), zero-warning ESLint, the 8 GB TypeScript profile, the 8 GB production build, postbuild output verification, and pinned native/WASM `rhwp` tracing. Latest-main branch CI also passes Vercel, code quality, TypeScript/Vitest, build budget, security, dependency and product-registration policy checks.
- This candidate was implemented in an isolated worktree and applied only to the isolated Supabase project `product-registration-v61-rehearsal`. Normal, identical-retry, malformed-price rollback, immutable-mutation, blocked-product, service-role fail-closed catalog, and durable four-product/shared-price checks pass there. The Clark baseline binds one exact source hash to four revisions, 54 scoped departures and 54 immutable SELLING-price lineage rows with no excluded-date leakage.
- Protected Preview commit `161ef4991` completed authenticated 390×844 package, LP and CTA proof without browser-visible server credentials. A verified signed token is exchanged for a maximum ten-minute HttpOnly/Secure/Strict package-path cookie; proof redirects and both final surfaces are no-store/noindex. Public catalog stayed at zero because the rehearsal canary snapshot is stale and still uses logo media. P0 mechanics are implementation-ready, but the product data is not sale-ready.
- Production database writes, production deployment, public pointer changes and auto-publish changes remain zero/off. Production remains `publication_freeze=true` and `PRODUCTION READY: NO` until the two Danang products are rebuilt from source, shadow convergence/live-domain canary pass, and rollout is explicitly approved.
