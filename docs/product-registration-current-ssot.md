...
- Product price recovery is now cross-checked by a price red-team audit (`src/lib/product-registration/price-red-team-auditor.ts`). If source-backed reader dates and recovered product prices overlap but disagree on amount, or if source-backed dates are completely disjoint from recovered dates, the customer publish gate receives a blocker. Gemini/LLM fallback can assist extraction, but it cannot bypass source-backed price/date verification.
- Micro QA writes the full four-phase ledger for every upload, including clean first-pass registrations. The phase names are `normal_registration`, `deterministic_source_recompare`, `render_payload_audit_repair`, and `final_reregistration_deliverability_audit`. Clean uploads use the last three phases as read-only verification passes; blocked or dirty uploads may apply deterministic repairs during the bounded three repair phases.
