...
- `Safe to withdraw` is the lower of (a) cash remaining after open-trip reserves, tax reserve, and unresolved positive cash and (b) unspent after-tax classified operating profit. Any open booking with missing supplier cost, unallocated travel row, ledger/bank drift, or unclassified non-travel cash blocks withdrawal and displays the reason.
- Open-trip cash reserve is calculated per booking as the greater of refundable customer cash currently held and known unpaid supplier cost. Missing supplier cost keeps the booking blocked and the displayed shortfall is only a minimum, never an exact final liability.
...
- Departure-month cash close is an explicit owner approval, not an automatic date transition. The preview may close unconfirmed bookings through the selected completed month without `total_price` or `total_cost` only when authoritative Clobe 4128 allocations are complete and `deposit - withdrawal > 0`. Zero/negative margin, missing bank evidence, allocation drift, cancelled, and deleted bookings remain unconfirmed for review. Recompute immediately before applying, require the candidate set to match the reviewed preview, preserve bank/allocation evidence, set `settlement_mode='cash'`, and write an audit log.
