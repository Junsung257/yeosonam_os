...
# ERR-20260831-04 — Route rewrite mixed excluded facts and false quality signals

- **Symptom**: The second Guam airport-to-Tumon canary rewrite still listed evidence instead of choosing a transport mode. It was also blocked for a stale no-fee claim, a `$5` template artifact, early Kakao CTA, broad destination duplicate, and weak route structure.
- **Root cause**: The full decision artifact was appended after the filtered rewrite packet, so excluded cancellation facts re-entered the prompt. Exact copied claims kept the model's generic ledger type instead of the persisted type. The deterministic artifact used the reader question as its answer. Quality regexes treated any one-digit dollar amount and any occurrence of `카카오` as defects, while the micro angle was not durable in generation metadata.
- **Fix**: Restrict rewrite artifacts to the approved subset, canonicalize exact ledger entries, classify no-fee cancellation as price before duration, insert a declarative route decision, require 승차/이동 구간/하차 stages, preserve or infer the micro angle, and narrow artifact/CTA detection to real residue or consultation language. V8 grants only the exact V7-controlled quarantine one corrected `rewrite_pro_max` attempt.
- **Regression proof**: Focused tests cover the production no-fee sentence, `$5`, Kakao T Guam Taxi, route fact selection and deduplication, deterministic opening, artifact filtering, micro-angle handoff, and exact-vs-lookalike V8 recovery. The combined suite passes 286 tests plus TypeScript checking.
