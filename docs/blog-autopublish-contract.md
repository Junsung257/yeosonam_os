...
- A research-ready local-transport article must pass through the deterministic final structure repair before its quality gate. The repair may use only approved claims and their linked official sources, and must materialize at least two distinct route or usage rows with verified prices, two verified duration or wait-time rows, schedule checks, ticket/reservation guidance, service limits, source links, the research check date, and evidence-derived FAQ entries for structured data. Missing evidence must leave the public article unchanged; it must never be filled with an invented timetable, headway, fare direction, or pass price. An SEO block must persist the full component scorecard in `meta.last_seo_score`, not only the aggregate score, so repeat failures are diagnosable without regenerating the article.
