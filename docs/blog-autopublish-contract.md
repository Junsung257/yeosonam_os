...
- Every publisher AI provider call has its own network deadline inside the larger writer deadline. The primary Gemini attempt gets 30 seconds and the DeepSeek fallback gets 50 seconds; SDK retries are disabled for these bounded attempts. A stalled primary must be aborted so fallback can run, and the configured policy provider must not be called a second time when that same provider already failed in the cascade.
