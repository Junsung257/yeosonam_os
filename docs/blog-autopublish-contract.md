...
Every quality-gate rerun must apply generic customer-surface and keyword-density cleanup before the final deterministic research-structure repair. The research repair is the last body mutation before image restoration, literal-newline normalization, and quality evaluation, so a generic lead rewrite cannot delete verified high-risk entry evidence. This order applies to both ordinary and AI-readability reruns and is enforced by route contract tests.
