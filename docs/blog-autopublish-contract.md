...
> 2026-09-02 research-ready refill addendum: 관측 수요는 필요조건일 뿐 충분조건이 아니다. 프로그램형 후보는 matrix angle을 명시적 `micro_angle`·`audience`·결정론적 `expected_slug`로 변환하고, 콘텐츠 계약 통과, 무인 발행 허용 위험도, 목적지·인텐트에 맞는 active reviewed source coverage, active representative 부재를 모두 확인한 뒤에만 큐에 들어간다. generic budget처럼 현재 정보 계약이 없는 angle, 사람 검수가 필수인 visa/entry, 출처가 없는 목적지 조합, 이미 대표 글이 있는 조합은 대기 풀에 남기고 큐 슬롯을 소비하지 않는다. 기존 `programmatic-seo-generator` API도 별도 LLM·직접 INSERT를 하지 않고 이 공통 승격 서비스만 호출한다.
> 2026-09-02 dispatch-proof addendum: `blog-generate`는 큐의 priority 순서만으로 Inngest 이벤트를 보내지 않는다. 운영 진단과 같은 SSOT 선택기로 콘텐츠 계약, 연구 브리프, 관측 수요, active representative, 최근/큐 중복을 다시 검사하고 최대 200개 후보 풀에서 실제 발행 가능 행만 선택한다. 보호 shadow 릴리스는 이벤트 접수로 성공 처리하지 않으며, `approved_for_slot` run, 완료·승인 attempt, `published_at IS NULL`인 비공개 creative, 해당 creative의 색인 outbox 0건을 운영 DB에서 읽기 전용으로 증명해야 통과한다. 후보가 terminal 실패/skip이거나 15분 안에 이 증거가 완성되지 않으면 릴리스는 실패한다.
>
