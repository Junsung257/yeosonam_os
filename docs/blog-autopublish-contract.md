...
For `entry_requirements`, the final research-structure boundary must make the verified destination explicit as `목적 국가: <destination>` when the writer omitted that label. The value may come only from one unambiguous destination shared by the passed source/evidence bundle; conflicting or missing destinations remain blocked. This repair supplies structural context, not a visa or policy claim, and must be idempotent.
