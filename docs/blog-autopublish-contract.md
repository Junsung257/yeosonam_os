...
Image count and rewritten alt text are not proof of visual relevance. Pexels selection must use destination + intent queries, inspect the provider's original photo description, reject scene conflicts such as an unrelated coast for an inland city, and prefer the first relevant search page instead of random deep pages. If no candidate reaches the relevance threshold, keep the slot empty or use a separately verified OG image; never relabel an unrelated image with a destination caption. AI-generated images must use the current Gemini native image API, be persisted to an immutable public `blog-assets/generated/blog/...` Storage path before insertion, and carry `AI 생성 참고 이미지` in alt text. Base64/data URLs are never publishable blog image URLs, and generated visuals are illustrative context rather than factual evidence.
