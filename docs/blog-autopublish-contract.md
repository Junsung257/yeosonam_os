...
> 2026-08-27 autonomous V4 override: the scheduled chain is `blog-scheduler` (00:00 KST) → `blog-generate` (01:05~06:55 KST) → `blog-publication-controller` (publication slots) → independent `blog-indexing-worker` (every five minutes). A low-risk candidate that passes the final quality/evidence/public-surface contract is unattended. HIGH-risk topics are auto-quarantined before model invocation with no publication or indexing job; they are not held for human approval in the V4 factory lane.

> 2026-08-19 AI Control Plane P0 addendum: `BLOG_AI_CONTROL_PLANE_ENABLED=1`인 durable Blog V4 호출은 `invokeAi`와 `ai_budget_buckets`/`ai_call_reservations`/`ai_call_receipts`를 거친다. DeepSeek 외 provider·fallback·advisor·자동 Pro 승격은 금지하고, candidate당 Flash 1회와 Pro 1회만 허용한다. 예산 RPC 실패, 중복 idempotency, receipt settlement 실패는 유료 호출을 fail-closed한다. 기존 공용 gateway의 내부 retry와 자동 escalation은 명시적 opt-in이며 이 경로에는 적용되지 않는다. migration apply·workload key 분리·production enablement는 별도 승인 작업이다.

> 2026-08-19 Durable Content Factory addendum: `BLOG_CONTENT_FACTORY_ENABLED`는 기본 `false`이며, 활성화 시 관측·검증된 GSC/Naver/고객/상품/editor 신호만 query-intent cluster와 `blog_content_operations` 원장으로 승격한다. 동일 intent 대표문서가 있으면 새 URL이 아니라 material refresh를 선택한다. 각 operation은 하나의 Workflow DevKit 실행으로 수요 재검증→불변 상품 snapshot→공식 연구/claim→V3 brief→DeepSeek 최대 5회→승인 재고까지 진행하며 cron 요청은 모델 실행을 기다리지 않는다. 공개는 publication controller만 수행하고 `pilot_3/ramp_10/max_30`의 작업 상한 3/10/30과 신규 URL 상한 2/6/18을 함께 적용한다. 상업성 공개는 creative 전환과 indexing outbox를 단일 fenced RPC에서 처리하며 색인 성공 뒤 operation을 `indexed`로 기록한다. 세부 운영은 이 문서와 `docs/blog-ops-runbook.md`의 2026-08-19 절이 우선한다.
...
>
> 2026-08-16 DeepSeek-only V4 release addendum: 검토된 공식 URL을 직접 fetch한 연구 자료만 DeepSeek Pro가 구조화한다. 계산 cron은 DeepSeek Flash 초안 → 규칙/claim/중복 평가 → DeepSeek Pro high/max 제한 재작성으로 동작한다. Gemini, GPT, Claude, generic provider cascade와 검색 snippet grounding은 이 발행 경로에서 금지한다. 후보별 writer 모델 호출은 최대 5회(초안 1회 + 재작성/보완 4회)이고 공급자 호출 전 원자적 일일 비용 예약이 필수다. 점수·표현·구조 결함은 이 보완 예산 안에서 재작성하며, 사실 근거·충돌·고위험 승인·중복 차단은 계속 fail-closed다. 공개 controller는 모델을 호출하지 않으며, 저장된 선택 시도·90점·hard blocker/failure 0건을 다시 확인한다. 2026-08-19 factory 활성화 시 계산 cron은 KST 01:05~06:55에 10분 간격으로 부족한 승인 재고를 채우고, 공개는 KST 09:00~22:00의 10개 누적 슬롯을 사용한다. 공개 상한은 내구성 상태 원장의 `pilot_3 → ramp_10 → max_30`을 따르며 결측 관측값은 승격을 금지하고 심각 사고는 즉시 동결한다. 세부 운영은 `docs/runbooks/blog-deepseek-orchestrator-v4.md`가 우선한다.
