...
For `entry_requirements`, research readiness must contain explicit supported claims for both the permitted travel purpose and permitted stay duration before the writer can run. Missing semantic coverage triggers one bounded structured-research retry and otherwise remains blocked. At the final research-structure boundary, one unambiguous destination shared by the passed source/evidence bundle may be rendered as `목적 국가: <destination>`, and omitted purpose/stay context may reuse only the two evidence-backed claims from that passed bundle. Conflicting or missing destinations and unsupported purpose/stay details remain blocked. Destination repair supplies structural context rather than a visa or policy claim; reused purpose/stay claims are added to the writer claim ledger. All repairs must be idempotent.

Every quality-gate rerun must reapply the deterministic customer-surface repair after research, generic, or AI-readability repair. This prevents a later repair from reintroducing malformed Korean particles or mechanical customer-facing phrases after the earlier surface pass.
