...
- A private atomic upgrade must spend its bounded generation window on the evidence-backed writer pass. It reuses the existing canonical topic and therefore skips optional SERP enrichment, Chain-of-Density rewriting, and a separate cover-image request. The normal inline-image stage still creates a fresh contextual set, and its first valid asset becomes the cover when the writer did not provide one.
