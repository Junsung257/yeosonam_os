...
- Scheduler research is intent-balanced: one row from each supported intent is attempted before repeated weather or budget rows. It must keep preparing until both the numeric buffer and a minimum of five distinct research-ready intents are present. A network/time budget may limit attempts, but rows not attempted in that invocation remain queued.
