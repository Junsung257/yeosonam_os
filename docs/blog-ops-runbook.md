...
- The source-complete airport-transport repair is recheck version `blog-information-research-recheck-20260831-v5`. V5 adds the GRTA-plus-Kakao multi-mode fallback after the V4 production canary exposed a transient Guam Airport page fetch failure. Advance this value only with a newer deployed repair and matching regression evidence; the version change is what permits a previously exhausted canary to receive one bounded retry.
