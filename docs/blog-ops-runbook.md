...
- V16 replaces those undersized route tables with short fact lists after the live table-integrity gate correctly rejected one- and two-row tables. Deterministic revalidation also copies the V3 brief's fixed description into the public output so title/H1/description remain one metadata contract. V17 forces provider JSON mode for the independent editorial judge. Historical attempts that already spent the original and text-mode retry may use one separately ledgered `editorial_judge_structured_retry` under the same daily cap. V18 safely normalizes nested/object-array judge schemas and exact string booleans while still requiring all five dimensions; it honors any explicit overall failure and allows one final `editorial_judge_normalized_retry`. A successful parsed report is stored in the reservation receipt and reused without another judge call. Failed parses store a bounded, non-secret diagnostic preview and remain fail-closed.
