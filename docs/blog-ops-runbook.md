...
- Before adding a climate destination to the automatic fallback catalog, fetch the exact registered documents through the production direct-fetch path and run the deterministic adapter. The acceptance result must contain the exact destination, a normals period, 12 unique months, 12 composite claims, and either 12 complete single-source evidence rows or 24 paired temperature/precipitation evidence rows. Do not register a city because its page exists. The 2026-07-30 live review accepted WMO Xi'an (`cityId=239`) and the JMA Nagasaki, Shizuoka, and Yufuin 1991-2020 tables; WMO Zhangjiajie and Boracay were rejected because their climate tables were empty.
