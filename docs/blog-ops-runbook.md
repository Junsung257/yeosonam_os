...
- V16 replaces those undersized route tables with short fact lists after the live table-integrity gate correctly rejected one- and two-row tables. Deterministic revalidation also copies the V3 brief's fixed description into the public output so title/H1/description remain one metadata contract. If an editorial-judge provider call is billed but its response cannot be parsed, one `editorial_judge_retry` reservation is allowed under the same daily cap; a successful parsed report is stored in the reservation receipt and reused on later revalidation without another judge call. A second parse failure remains fail-closed.
