...
- The source-complete airport-transport repair is recheck version `blog-information-research-recheck-20260831-v8`. V8 preserves the V5 GRTA-plus-Kakao multi-mode fallback and V6/V7 bounded queue recovery, then permits exactly one `rewrite_pro_max` retry for the controlled canary quarantined by the known evidence-filter, claim-label, route-answer, `$5`, and Kakao-T gate defects. It requires the prior V7 recovery markers and the complete production failure signature; lookalike quality failures remain blocked. Advance this value only with a newer deployed repair and matching regression evidence.
- Route rewrites receive only the decision-relevant approved claim subset. The appended decision artifact must be restricted to that same subset, and exact copied claims must inherit their persisted claim type/risk even if the model emits a generic ledger label. Airport-route openings are declarative option/condition answers; the visible body must cover 승차, 이동 구간, 하차, an official-location recheck, and a disruption fallback without inventing stops, taxi base fare, or door-to-door duration.
...
- Programmatic producers must additionally prove observed demand, a resolvable explicit information contract, reviewed source coverage for the same destination and intent, and absence of an active representative. A legacy compatibility route must delegate to `promotePendingTopics()` instead of writing the queue directly.
...
- V4 does not treat a coverage gap as demand evidence. `queuePillarGeneration()` may insert a `source='pillar'` row only when a durable GSC, Naver, customer-question, active-product, operator-note ID, editor-seed, search-volume, or trend signal is supplied. The scheduler therefore records a missing pillar as skipped planning work instead of creating an unpublishable `queued` row. Publisher deferral remains only as compatibility handling for legacy pillar rows.
...
- `blog-information-research-recheck-20260831-v10` handles only the exact V9 canary quarantine signature. It removes the stale research bundle and decision artifact so reviewed-source research runs again, while preserving the same controlled queue and forcing the final bounded `rewrite_pro_max` stage.
- V10 adds the reviewed Guam Visitors Bureau transportation page for regulated taxi-meter rates and Tumon transit coverage, and retains Guam Airport's West Arrivals taxi-counter evidence. It does not treat a taxi directory as fare evidence and does not infer a door-to-door duration or GRTA boarding point.
- Before applying V10, dry-run the exact queue ID and require `scanned=1`, `counts.requeue=1`, `updated=0`. After deployment and migration, apply once with `--write`; any different failure signature remains blocked.
- V11 closes the transient-source and model-hallucination paths exposed by the immutable failed V10 run. Missing critical Guam airport URLs receive one fresh, registry-bounded focused fetch; the final route article is then rebuilt from the ten approved facts instead of publishing model-authored route prose. Confirm `auto_research.version=reviewed-source-direct-fetch-v3`, inspect `critical_source_retry_count`, `critical_source_recovered_count`, and `direct_source_failure_count`, and require the deterministic route marker plus a ten-entry writer ledger.
- V12 handles an official host that remains unreachable from the production runtime after the focused retry. It may load only an unexpired, at-most-30-day immutable source version whose exact URL and official registry id still match the active research document and whose acquisition is `reviewed_registry_snapshot`. Confirm `auto_research.version=reviewed-source-direct-fetch-v4`, inspect `critical_source_snapshot_fallback_count` and the bounded `direct_source_failure_samples`, and verify that snapshot-backed evidence is labeled `reviewed_registry_snapshot` rather than `reviewed_direct_fetch`. With all ten route facts present, research and article assembly are deterministic; the opening includes approved fare values and the article includes a relevant `/destinations` link before ordinary gates run.
- V13 accepts both the ordinary slash and Unicode fraction slash used by the reviewed Visit Guam snapshot (`1/4`, `1⁄4`) when recognizing the complete regulated taxi-meter sentence. Confirm `auto_research.version=reviewed-source-direct-fetch-v5` and that the decision artifact retains the single grounded claim containing 2.40 USD, the first-mile 4.00 USD, and each following 0.25-mile 0.80 USD before the deterministic route article is assembled.
- V14 removes repeated GRTA fact/citation cells from the deterministic route table and narrows the title to the supported taxi-location, taxi-meter, and GRTA-fare promise. A controlled canary at the three-call ceiling may request `route_template_dedup_v2`; this rebuilds the marked route block from the persisted ten-fact artifact, keeps `attempts=3`, makes zero model calls, and is eligible only for the exact V13 quality-pass/claim-pass duplicate-template failure signature.
- V15 corrects the V14 attempt-ceiling assumption: the live policy allows five calls, so the rejected revalidation became immutable attempt 4. The repaired request is now accepted only at `attempts=4` with that exact attempt's readability/internal-label/semantic-missing signature. It also prefixes GRTA link labels with Korean so they cannot resemble internal enum labels and splits the long route table into three sub-520-character tables; the stored fact sentences and source list remain unchanged.
- V16 replaces those undersized route tables with short fact lists after the live table-integrity gate correctly rejected one- and two-row tables. Deterministic revalidation also copies the V3 brief's fixed description into the public output so title/H1/description remain one metadata contract. V17 forces provider JSON mode for the independent editorial judge. Historical attempts that already spent the original and text-mode retry may use one separately ledgered `editorial_judge_structured_retry` under the same daily cap. V18 safely normalizes nested/object-array judge schemas and exact string booleans while still requiring all five dimensions and allows one final `editorial_judge_normalized_retry`. V19 makes those five required dimensions authoritative when a provider's redundant top-level `passed` contradicts them: any failed dimension still blocks, while five explicit passing dimensions cannot be vetoed by a stray top-level false. V20 applies the same evidence-backed canary description helper in both normal generation and evaluator-only revalidation so the immutable attempt CAS sees one title/H1/description contract. A successful parsed report is stored in the reservation receipt and reused without another judge call. Failed parses store a bounded, non-secret diagnostic preview and remain fail-closed.
- V21 synchronizes a non-human informational article's durable review case to `ready` with the exact final body/title/description/slug fingerprint before the run becomes `approved_for_slot`. Deferred publication fails closed when the research, claim validation, or human-review policy no longer passes. This prevents a stale `pending_review` case from quarantining an otherwise immutable approved attempt at the atomic public commit. A controller-failed controlled canary may also resume the already-revalidated attempt only when its zero-model-call audit, score, empty blockers, queue attempt count, and exact source-attempt id still match.
- A V11 canary must use a new queue/run rather than resetting the five immutable model-call attempts of the failed V10 run. The new topic must be a genuinely narrower reader task, not a suffix-only duplicate. Keep the old run quarantined as failure evidence, keep rollout frozen, and publish only after the new run passes the ordinary evidence, claim, V5 editorial, public-customer, duplicate, and strict publication gates.
...
For a protected one-item shadow smoke test, run the protected release with `run_shadow_smoke=true`. It calls the authenticated candidate dispatcher with `force=1&limit=1` only after readiness passes, verifies exactly one Inngest handoff, and stores a secret-free summary in the release artifact. The `limit` override is honored only for forced manual runs and is hard-capped at two; scheduled runs remain policy-controlled. If Deployment Protection is enabled, the same current bypass key must exist as the server-only `VERCEL_AUTOMATION_BYPASS_SECRET` so the Inngest worker can call its own protected cron route.

Before dispatch, the common queue preflight preserves any stored terminal failure as the audit cause. It deterministically marks a `generic_unmarked` destinationless information topic as `intentionally_generic`, uses `해외여행 공통` as its research evidence scope, and skips genuinely missing or invalid destination rows. A non-retryable draft result such as `evidence_insufficient`, a real duplicate, or a contract failure must be returned as terminal on the first call; it must not consume Inngest retries or continue to artifact verification.

Before dispatch, `blog-generate` settles queued rows that the shared failure policy already considers terminal and filters the candidate pool with the same policy. `information_representative_preclaim:*` is a canonical ownership collision, so it is `skipped` immediately and must never be rescheduled ahead of a fresh candidate. A controlled operator run may pass `targetQueueId` only together with `force=1|true`; the dispatcher must select that exact eligible row or return `target_queue_item_not_dispatchable` without falling back to another row.
