...
- Scheduler research is intent-balanced: one row from each supported intent is attempted before repeated weather or budget rows. A network/time budget may limit attempts, but rows not attempted in that invocation remain queued.
