...
- V16 replaces those undersized route tables with short fact lists after the live table-integrity gate correctly rejected one- and two-row tables. Deterministic revalidation also copies the V3 brief's fixed description into the public output so title/H1/description remain one metadata contract. V17 forces provider JSON mode for the independent editorial judge. Historical attempts that already spent the original and text-mode retry may use one separately ledgered `editorial_judge_structured_retry` under the same daily cap. A successful parsed report is stored in the reservation receipt and reused without another judge call; a structured retry failure remains fail-closed.
