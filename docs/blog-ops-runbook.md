...
- Live publishing now uses the same scorecard as repair input. `repairBlogEngineCategoryGaps()` writes `generation_meta.engine_category_repair` with before/after score, repaired categories, and repair actions so operators can see whether weak customer-facing categories were fixed before publish instead of only blocked.
