...
- At 21:45 KST Vercel and 21:50 KST the external scheduler call `blog-regenerate-zero-click`. The route queues no more than two published in-place upgrades. When 14-day rank data is absent it falls back to missing-research legacy posts, while preserving explicit-intent classification, representative ownership, high-risk review, cooldown, and atomic replacement gates.
