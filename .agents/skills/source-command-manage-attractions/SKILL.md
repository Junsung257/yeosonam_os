---
name: "source-command-manage-attractions"
...
# source-command-manage-attractions

Use this skill when the user asks to run the migrated source command `manage-attractions`.
...
5. **설명 자동 보강** — 사장님 등록한 attraction 의 short_desc/long_desc 가 비어 있을 때 CSV 다운로드 → 외부 Codex → 업로드 (사장님 주도)
...
기능 부재. 사장님이 외부 Codex에서 직접:
1. 관리자 페이지에서 CSV 다운로드
2. 외부 Codex에게 "각 관광지 short_desc/long_desc 작성" 요청
3. 엑셀 편집
4. 관리자 페이지에서 CSV 업로드
