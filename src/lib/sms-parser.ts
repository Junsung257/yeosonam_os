...
export interface ParsedSMS {
  ...
  isWithdrawal: boolean;     // 출금/송금 여부
  transactionType: '입금' | '출금' | null;
...
function cleanCounterpartyName(name: string | null | undefined): string | null {
  if (!name) return null;
  const cleaned = name
    .replace(/\[[^\]]+\]/g, ' ')
    .replace(/(?:^|\s)(Web발신|신한은행|Shinhan|입금통보|출금통보|입금|출금|이체출금|자동이체|송금|잔액)(?=\s|$)/g, ' ')
    .replace(/[·•]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  if (!cleaned || ['신한', '국민', '은행', '입금', '출금', '잔액'].includes(cleaned)) {
    return null;
  }
...
export function parseShinhanSMS(smsText: string, receivedAt?: Date): ParsedSMS {
  ...
  const isDeposit = /입금/.test(text) && !/출금/.test(text);
  const isWithdrawal = /출금|이체출금|자동이체|송금/.test(text);
  const transactionType: '입금' | '출금' | null = isDeposit ? '입금' : isWithdrawal ? '출금' : null;

  // 금액 추출: 숫자,숫자원 패턴
  const amountPatterns = transactionType === '출금'
    ? [
        /([0-9,]+)원\s*(?:출금|이체출금|자동이체|송금)/,
        /(?:출금|이체출금|자동이체|송금)\s*(?:[^\d원]{0,40})?\s*([0-9,]+)원/,
      ]
    : [
        /([0-9,]+)원\s*입금/,
        /입금\s*(?:[^\d원]{0,40})?\s*([0-9,]+)원/,
        /([0-9,]+)원\s*(입금)/,
      ];
  ...
  // 신한은행 태그 이후 이름 추출
  const nameToken = '[가-힣A-Za-z0-9㈜()·.\\-\\s]{2,40}?';
  const namePatterns = transactionType === '출금'
    ? [
        new RegExp(`\\[신한은행\\]\\s*(?:출금|이체출금|자동이체|송금)\\s*(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`(?:출금통보|출금|이체출금|자동이체|송금)\\s+(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`(${nameToken})\\s+[\\d,]+원\\s*(?:출금|이체출금|자동이체|송금)`),
      ]
    : [
        new RegExp(`\\[신한은행\\]\\s*(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`\\[Web발신\\]\\[신한은행\\]\\s*입금\\s*(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`입금통보\\s+(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`입금\\s+(${nameToken})\\s+[\\d,]+원`),
        new RegExp(`(${nameToken})\\s+[\\d,]+원\\s*입금`),
        new RegExp(`(${nameToken})\\s+입금`),
      ];

  for (const pattern of namePatterns) {
    const match = text.match(pattern);
    if (match?.[1]) {
      const candidate = cleanCounterpartyName(match[1]);
      if (candidate) {
