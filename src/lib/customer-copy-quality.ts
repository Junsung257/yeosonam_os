...
const QUALITY_RULES: Array<{ code: string; pattern: RegExp; label: string }> = [
  ...
  },
  {
    ...
    pattern: /\bTAX\s*\(\s*\d{1,2}\s*월\s*기준\s*\)|유류할증료\s*\(\s*\d{1,2}\s*월\s*기준\s*\)|\d{1,2}\s*월기준|\d{1,2}\s*월\s*(?:선발권\s*기준\s*요금|(?:선)?발권(?!\s*기준))|기사가이드경비|기사\s*가이드\s*경비|\[\s*[A-Z0-9]{2,3}\s+[^\]]*?PKG\s*\]|(^|[\s📍])\[?[A-Z0-9]{2,3}\]\s*(?=[가-힣])|[☑✔ώ]|✓(?=\S)|(?<=\S)✓|[가-힣]+\s*OR\s*[가-힣]+|[가-힣]+\s*or\s*[가-힣]+|바나산\s*정산|맥주\s*OR\s*음료/i,
...
export function normalizeCustomerVisibleCopy(value: string | null | undefined): string {
  const decoded = decodeCustomerHtmlEntities(value)
    ...
    .replace(/유류할증료\s*\(\s*(\d{1,2})\s*월\s*(?:선)?발권\s*\)/g, '유류할증료 $1월 발권 기준')
    .replace(/\bTAX\s*\(\s*(\d{1,2})\s*월\s*(?:선)?발권\s*\)/gi, '항공세 $1월 발권 기준')
