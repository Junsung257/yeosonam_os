...
- **편집자 판단 문장**: 글 전체에 최소 3곳, 자연스럽게 분산. 단순 비교를 넘어 개인적인 관점을 느끼게. 예: "같은 가격대 다낭 상품 중 바나산 2시간 체류는 흔하지 않아요."

1. **마크다운 출력** (# H1, ## H2). 코드블록(\`\`\`)으로 감싸지 말 것.
2. **H1**은 Brief.h1 그대로 사용.
3. **H1 바로 아래, 핵심 요약보다 먼저 정의문 도입부 1문단을 넣을 것** (ai_readability 게이트 통과).
   - "~이란/~은/~는 …입니다/이에요" 형식으로 글의 대상을 정의하는 문장 1개
   - 이어서 독자의 궁금증을 유발하는 문장 1개
   - 예: "다낭 3박5일 패키지란 부산 출발 베트남 중부 다낭·호이안을 둘러보는 일정이에요. 바나산 케이블카와 호이안 야시장이 하이라이트죠."
4. **정의문 직후 "## 핵심 요약" 섹션을 필수로 넣을 것** (TL;DR).
   - \`## 핵심 요약\` 제목에 이어 bullet 3~5개 (\`- \` 로 시작)
   - 각 bullet은 20자~40자 내외, 숫자·고유명사 포함 (예: "- 부산(김해) 출발 에어부산 직항 3박5일")
   - AI Overviews/SGE가 인용하기 좋은 사실 중심 문장으로
5. **H2 제목은 예시처럼 \`[대괄호]\` 스타일 권장** (예: \`## [1일 차 | 천문산]\`, \`## [여소남이 이 상품을 엄선한 이유]\`).
6. **질문형 H2 1개 이상 필수 포함**
   - H2 제목이 ? 로 끝나는 섹션을 1개 이상 만들 것
   - 예: "## [혼자 가도 괜찮을까?]", "## [환전은 달러 vs 원화?]"
   - FAQ 직전이나 팁 섹션 앞에 자연스럽게 배치
7. 각 H2는 Brief의 "씨앗 메시지"를 4~7문장으로 확장.
8. **상품 팩트(가격/박수/호텔명/관광지명/항공편 시간)는 절대 변경 금지**. 확실치 않으면 언급하지 말 것.
9. 각 H2 바로 다음 줄에 지정된 이미지를 **그대로 복사**해 삽입. 중복 삽입 금지.
   - 이미지 alt는 \`![목적지 - 섹션명](URL)\` 형식 권장 (예: \`![다낭 - 바나산 국립공원](...)\`)
10. 문장은 60자 이내로 짧게. 한 문단은 3~5문장.
11. **항공 스케줄/요금/포함사항은 마크다운 표**로 작성 (상품 팩트가 있는 경우).
12. **"## [자주 묻는 질문]" 섹션을 CTA 직전에 필수로 넣을 것** (FAQPage 리치 스니펫 + 롱테일 검색 유입 목적).
...
13. 마지막에 \`## [여행 준비를 위한 실전 팁]\` 섹션 추가 (여권 / 복장 / 비자 / 통신 / 상비약 등).
14. **외부 권위 링크 2개 이상 포함** (정부 기관·관광청·공항 공사·공식 사이트). 신뢰도 낮은 개인 블로그/카페 금지.
15. 마지막 CTA는 위 지정 문장 + 버튼: \`**[👉 ${variations.cta_button_label}](${productUrl})**\`
16. 해시태그 15개를 맨 마지막 줄에 한 줄로 (Few-shot 예시 형식 참고).
17. 전체 분량 2200~3200자.
