...
function sourceCellPrice(value: string): SourceCellPrice | null {
  ...
  // Some supplier HWP cells render a struck/stacked list price above the
  // actual sale price with a down-arrow glyph instead of a textual label.
  // The visual order is authoritative only for this exact two-amount shape.
  if (/(?:↴|↡|↓)/u.test(value) && candidates.length === 2) {
    const [listPrice, finalPrice] = candidates;
    return {
      amount: finalPrice!.amount,
      listPrice: listPrice!.amount,
      priceRelation: 'final_sale',
      sourceAmountScale: finalPrice!.sourceAmountScale === 1000 ? 1000 : 1,
    };
  }
...
function parsePairedMonthDayPriceGrid(
  ...
  for (const [row, cells] of rows.entries()) {
    ...
    // A merged HWP cell covers every physical row in its rowSpan. Month labels
    // still advance only on their origin row, but the adjacent date roster and
    // stacked normal/sale price must remain available on each covered row.
    const coveredByColumn = new Map<number, DocumentIrTableCell>();
    for (const cell of [...coveredCells].sort((left, right) => (
      left.column - right.column
      || right.row - left.row
      || left.rowSpan - right.rowSpan
    ))) {
      coveredByColumn.set(cell.column, cell);
    }
    for (const amountCell of coveredByColumn.values()) {
      const price = sourceCellPrice(amountCell.text);
      if (!price) continue;
      const dateCell = coveredByColumn.get(amountCell.column - 1);
      if (!dateCell) continue;
