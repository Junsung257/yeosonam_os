...
export function buildProductRegistrationV5ConvergenceRows(input: {
  ...
  const lineage = input.payload.catalog_product_id
    ? { catalog_product_id: input.payload.catalog_product_id }
    : {};
  const rows: ProductRegistrationV5ConvergenceRow[] = [
    { ...lineage, package_id: input.payload.package_id, snapshot_id: input.payload.snapshot_id, snapshot_hash: input.payload.snapshot_hash, surface: 'packages', route: packageRoute, status: 'pending' },
    { ...lineage, package_id: input.payload.package_id, snapshot_id: input.payload.snapshot_id, snapshot_hash: input.payload.snapshot_hash, surface: 'lp', route: lpRoute, status: 'pending' },
    { ...lineage, package_id: input.payload.package_id, snapshot_id: input.payload.snapshot_id, snapshot_hash: input.payload.snapshot_hash, surface: 'og', route: `/api/og/affiliate?pkg=${encodeURIComponent(input.payload.package_id)}`, status: 'pending' },
    { ...lineage, package_id: input.payload.package_id, snapshot_id: input.payload.snapshot_id, snapshot_hash: input.payload.snapshot_hash, surface: 'affiliate', route: `/api/og/affiliate?pkg=${encodeURIComponent(input.payload.package_id)}`, status: 'pending' },
  ];
  if (input.shortCode && input.shortCode !== input.payload.package_id) {
    rows.splice(2, 0, { ...lineage, package_id: input.payload.package_id, snapshot_id: input.payload.snapshot_id, snapshot_hash: input.payload.snapshot_hash, surface: 'lp', route: `/lp/${encodeURIComponent(input.shortCode)}`, status: 'pending' });
  }
  return rows;
}
