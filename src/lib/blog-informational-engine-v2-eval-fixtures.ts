...
export const BLOG_INFORMATION_ENGINE_V2_EVAL_FIXTURES: BlogInformationEngineV2EvalFixture[] = [
  ...
  {
    id: 'canadian_rockies_local_transport',
    label: '캐나다 로키 현지 교통',
    slug: 'canadian-rockies-local-transport-safety-eval',
    plannerInput: {
      intentType: 'local_transport',
      topic: '캐나다 로키 렌터카 없이 대중교통 이동',
      destination: '캐나다 로키산맥',
      primaryKeyword: '캐나다 로키 대중교통',
    },
    expectedIntent: 'local_transport',
    expectedCtaKeys: ['NAVER_CAFE', RELATED],
    expectedPublishState: 'published',
  },
  {
  ...
  },
  {
    id: 'danang_itinerary',
    label: '다낭 여행 일정',
    slug: 'danang-itinerary-safety-eval',
    plannerInput: {
      intentType: 'itinerary',
      topic: '다낭 3박 4일 여행 일정과 동선',
      destination: '다낭',
      primaryKeyword: '다낭 여행 일정',
    },
