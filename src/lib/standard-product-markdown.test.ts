...
import { isStandardProductMarkdown, parseStandardProductMarkdown, STANDARD_PRODUCT_MARKDOWN_TEMPLATE } from './standard-product-markdown';
...
- 출발공항: 부산/김해
- 항공: LJ
- 출발편: LJ115 21:35 부산/김해 -> 00:25 나트랑
- 귀국편: LJ116 01:00 나트랑 -> 06:40 부산/김해
- 출발요일: 매주 목요일
- 최소출발: 6명
- 발권마감: 출발 7일 전
- 랜드사: 씨어코코
...
- 21:35 | LJ115 부산/김해 출발 | flight
- 00:25 | 나트랑 도착 | flight

### DAY 2 | 나트랑, 달랏 | 달랏 5성 호텔(5성) | 조식 호텔식 / 중식 현지식 / 석식 현지식
...
- POLICY | 취소/변경 | 취소료는 여행 약관과 항공사 규정에 따라 적용됩니다.
- INFO | 현지 안내 | 현지 사정과 항공 스케줄에 따라 일정 순서가 변경될 수 있습니다.
...
describe('standard product markdown', () => {
  ...
  it('ships a readable Korean operator template', () => {
    expect(STANDARD_PRODUCT_MARKDOWN_TEMPLATE).toContain('## 기본정보');
    expect(STANDARD_PRODUCT_MARKDOWN_TEMPLATE).toContain('- 상품명:');
    expect(STANDARD_PRODUCT_MARKDOWN_TEMPLATE).toContain('## 일정');
    expect(STANDARD_PRODUCT_MARKDOWN_TEMPLATE).not.toContain('?곹뭹');
  });

  it('converts Korean markdown into parser output without LLM usage', () => {
    ...
    expect(parsed.extractedData.destination).toBe('나트랑');
...
## 일정
### DAY 5 | 나트랑, 부산 | 기내박(기내) | 조식 X / 중식 X / 석식 X
- 10:00 | 죽림선원 관광 | normal | 관광지ID: 5728e681-636b-42fa-87b5-a2f0b7b0379c`);
  ...
  it('keeps customer render claims backed by the markdown source', () => {
    ...
    const intake = pkgToIntake(pkg as Parameters<typeof pkgToIntake>[0], { landOperatorName: '씨어코코' });
