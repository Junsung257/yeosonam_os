...
describe('canAutoMatchSettlementMemo', () => {
  it('allows an exact settlement key and only explicitly allows a newly created booking', () => {
    expect(canAutoMatchSettlementMemo({ bookingId: 'booking-1', source: 'existing_key', confidence: 1 })).toBe(true);
    expect(canAutoMatchSettlementMemo({ bookingId: 'booking-2', source: 'created_booking', confidence: 1 })).toBe(false);
    expect(canAutoMatchSettlementMemo({ bookingId: 'booking-2', source: 'created_booking', confidence: 1, allowCreatedBooking: true })).toBe(true);
  });
