import { isRealPerk } from '@/lib/render-contract';
import { hasCustomerCopyQualityIssues } from '@/lib/customer-copy-quality';

interface CopyInput {
  title?: string | null;
  destination?: string | null;
  duration?: number | null;
  trip_style?: string | null;
  departure?: string | null;
  product_type?: string | null;
  inclusions?: string[] | null;
  product_highlights?: string[] | null;
  airline?: string | null;
}

const INTERNAL_COPY_PATTERN =
  /(배포|스팟|선발특가|초특가|마감임박|특가\s*배포|\d{1,2}\s*\/\s*까지|요금표|행사코드|판매가\s*문의)/gi;

const COST_COUNSELING_PATTERN =
  /(현지에서\s*따로\s*드는\s*비용|상담\s*때\s*한\s*번에\s*정리|추가\s*비용은\s*상담|문의\s*시\s*안내)/i;

const MECHANICAL_COPY_PATTERN =
  /(상품입니다[.\s]*(?:.+?)중심\s*일정으로\s*구성됩니다|왕복\s*항공과(?:.+?)숙박(?:.+?)중심\s*일정|패키지\s*상품\s*상세\s*페이지)/i;

function cleanText(value: string | null | undefined): string {
  return String(value ?? '')
    .replace(INTERNAL_COPY_PATTERN, '')
    .replace(/\s+/g, ' ')
    .trim();
}

function formatDuration(input: CopyInput): string | null {
  const tripStyle = cleanText(input.trip_style);
  if (tripStyle && tripStyle !== 'UNK') return tripStyle;
  if (input.duration && input.duration > 0) return `${input.duration}일`;
  return null;
}

function labelDestination(input: CopyInput): string {
  const dest = cleanText(input.destination);
  if (dest && dest !== 'UNK') return dest;

  const title = cleanText(input.title);
  const firstToken = title
    .split(/\s+/)
    .find(token => token && !/^(BX|KE|LJ|TW|7C|ZE|OZ|RS|PUS|ICN)$/i.test(token));
  return firstToken || '여행지';
}

function eulReul(value: string): string {
  const last = value.charCodeAt(value.length - 1);
  if (last < 0xac00 || last > 0xd7a3) return `${value}를`;
  return `${value}${(last - 0xac00) % 28 === 0 ? '를' : '을'}`;
}

function pickHighlights(input: CopyInput): string[] {
  const out: string[] = [];
  if (input.product_highlights && input.product_highlights.length > 0) {
    out.push(...input.product_highlights.map(cleanText).filter(Boolean).slice(0, 2));
  }
  if (out.length < 2 && input.inclusions) {
    const perks = input.inclusions
      .map(cleanText)
      .filter(s => s && isRealPerk(s))
      .slice(0, 2 - out.length);
    out.push(...perks);
  }
  return out;
}

function haystack(input: CopyInput): string {
  return [
    input.title,
    input.destination,
    input.product_type,
    input.airline,
    ...(input.product_highlights ?? []),
    ...(input.inclusions ?? []),
  ]
    .map(cleanText)
    .join(' ');
}

function isGolf(input: CopyInput): boolean {
  return /(골프|라운딩|골프텔|골프장|\bCC\b|C\.C|country\s*club|golf)/i.test(haystack(input));
}

function isFerry(input: CopyInput): boolean {
  const type = cleanText(input.product_type).toLowerCase();
  return type === 'cruise' || type === 'ferry' || /(페리|훼리|선박|배편|크루즈|부관훼리|팬스타)/i.test(haystack(input));
}

function isOnsen(input: CopyInput): boolean {
  return /(온천|료칸|유노하나|벳부|구로카와|쿠로가와|노천탕|온천욕)/i.test(haystack(input));
}

function extractGolfStayPhrase(input: CopyInput): string | null {
  const dest = labelDestination(input);
  const tokens = cleanText(input.title)
    .split(/\s+/)
    .filter(token => token && token !== dest && !/^(BX|KE|LJ|TW|7C|ZE|OZ|RS|에어부산|제주항공|진에어|티웨이|아시아나)$/i.test(token));

  const idx = tokens.findIndex(token => /(골프텔|골프장|라운딩|\bCC\b|C\.C)/i.test(token));
  if (idx >= 0) {
    return tokens.slice(Math.max(0, idx - 1), idx + 1).join(' ');
  }

  const fromHighlights = pickHighlights(input).find(item => /(골프텔|골프장|라운딩|\bCC\b|C\.C)/i.test(item));
  if (fromHighlights) return fromHighlights.replace(/[.,]?\s*(포함|이용|숙박).*$/g, '').trim();

  return null;
}

function joinLines(lines: string[]): string {
  return lines.map(line => line.trim()).filter(Boolean).join('\n\n');
}

export function generateRecommendationCopy(input: CopyInput): string {
  const destination = labelDestination(input);
  const duration = formatDuration(input);
  const highlights = pickHighlights(input);

  if (isGolf(input)) {
    const stay = extractGolfStayPhrase(input) ?? `${destination} 골프`;
    return joinLines([
      `⛳ ${destination}에서 라운딩과 휴식을 함께 챙기는 일정입니다.`,
      `🏌️ ${stay} 중심으로 동선을 잡아 이동 부담을 줄이고, 라운딩 사이에 쉬어갈 시간을 남겼습니다.`,
      `🌿 ${duration ? `${duration} 동안` : '여행하는 동안'} 골프 일정과 숙박 흐름을 함께 확인할 수 있어 예약 전 구성 파악이 쉽습니다.`,
    ]);
  }

  if (isFerry(input)) {
    const carrier = cleanText(input.airline);
    return joinLines([
      `🛳️ ${carrier ? `${carrier}로 ` : ''}${destination}까지 이동하는 구간을 포함한 일정입니다.`,
      `📍 ${highlights[0] ? `${highlights[0]} 등 주요 코스` : '주요 코스와 이동 동선'}를 일정표 순서대로 확인할 수 있습니다.`,
      `🌊 ${duration ? `${duration} 동안` : '짧은 일정에도'} 이동 부담은 줄이고 관광 흐름이 이어져 예약 전에 전체 일정을 파악하기 좋습니다.`,
    ]);
  }

  if (isOnsen(input)) {
    return joinLines([
      `♨️ ${destination}에서 관광과 온천 휴식을 함께 보는 일정입니다.`,
      `📍 ${highlights[0] ? `${highlights[0]}를 중심으로` : '숙박과 이동 흐름을 중심으로'} 무리하지 않게 둘러보는 구성을 확인할 수 있습니다.`,
      `🧖 ${duration ? `${duration} 동안` : '여행 중'} 볼거리와 쉬어가는 시간이 함께 배치된 ${destination} 여행입니다.`,
    ]);
  }

  return joinLines([
    `✈️ ${destination}의 주요 일정과 이동 흐름을 한 화면에서 확인할 수 있는 패키지입니다.`,
    `📍 ${highlights[0] ? `${highlights[0]} 중심 일정` : '주요 관광지와 이동 동선'}을 날짜별로 정리해, 예약 전에 핵심 코스를 먼저 파악할 수 있습니다.`,
    `🧭 ${duration ? `${duration} 동안` : '여행 중'} 항공, 숙박, 식사 조건을 함께 보며 전체 구성을 비교하기 좋습니다.`,
  ]);
}

export function isWeakCopy(copy: string | null | undefined, title?: string | null): boolean {
  const rawCopy = String(copy ?? '');
  if (INTERNAL_COPY_PATTERN.test(rawCopy)) {
    INTERNAL_COPY_PATTERN.lastIndex = 0;
    return true;
  }
  INTERNAL_COPY_PATTERN.lastIndex = 0;

  const cleanCopy = cleanText(copy);
  if (!cleanCopy || cleanCopy.length < 20) return true;
  if (COST_COUNSELING_PATTERN.test(cleanCopy)) return true;
  if (MECHANICAL_COPY_PATTERN.test(cleanCopy)) return true;
  if (hasCustomerCopyQualityIssues(cleanCopy)) return true;
  if (/^[가-힣A-Za-z0-9\s/·+().-]+패키지\s*여행$/.test(cleanCopy)) return true;

  const cleanTitle = cleanText(title);
  if (cleanTitle && cleanCopy.includes(cleanTitle) && cleanCopy.length < cleanTitle.length + 35) {
    return true;
  }

  return false;
}
