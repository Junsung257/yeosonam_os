...
const REGION_KEYWORDS = new Set([
  '인천','김포','부산','제주',
  '청도','칭다오','대만','타이베이','상해','북경','계림','양삭','서안','장가계','연길','도문','용정','이도백하',
...
function selectFlightColumnTimes(body: string, dayTimes: string[]): { depTime: string | null; arrTime: string | null } {
  if (dayTimes.length === 0) return { depTime: null, arrTime: null };

  // Supplier tables often include an airport meeting time before actual flight
  // departure/arrival columns. In that layout the last two times belong to
  // the flight; the first time is only the meeting guide.
  const hasPreFlightMeetingTime =
    dayTimes.length >= 3 &&
    /미팅|집결|수속|출발\s*\d+\s*시간\s*전/.test(body);

  if (hasPreFlightMeetingTime) {
    return {
      depTime: dayTimes[dayTimes.length - 2] ?? null,
      arrTime: dayTimes[dayTimes.length - 1] ?? null,
    };
  }

  return {
    depTime: dayTimes[0] ?? null,
    arrTime: dayTimes[1] ?? null,
  };
}
...
function extractTimes(body: string): string[] {
  ...
  for (const line of body.split('\n')) {
    const m = line.trim().match(/^(\d{1,2}:\d{2})(\+\d)?$/);
    if (!m) continue;
    out.push(m[1] + (m[2] ?? ''));
  }
  return out;
...
function extractSchedule(body: string, dayFlightCode: string | null, dayTimes: string[]): DayTableSchedule[] {
  ...
  const { depTime, arrTime } = selectFlightColumnTimes(body, dayTimes);
  ...
  for (const line of lines) {
    ...
    const cityFlightLine = /^[가-힣A-Za-z()\/]+(?:\s+[가-힣A-Za-z()\/]+)?\s*(?:출발|도착)(?:\s|$)/.test(line);
    const isAirportDep = /(국제)?\s*공항\s*출발/.test(line) || (/출발/.test(line) && !/도착/.test(line) && (/공항|김해|인천|김포/.test(line) || cityFlightLine));
    const isAirportArr = /(국제)?\s*공항\s*도착/.test(line) || (/도착/.test(line) && !/출발/.test(line) && (/공항|팡라오|김해|인천/.test(line) || cityFlightLine));
    if (isAirportDep || isAirportArr) {
      out.push({
        time: (isAirportDep ? depTime : arrTime) ?? undefined,
        activity: line,
...
export function parseDayTable(rawText: string): DayTableResult {
  ...
  const firstTimes = selectFlightColumnTimes(blocks[0].body, extractTimes(blocks[0].body));
  const lastTimes = blocks.length > 1
    ? selectFlightColumnTimes(blocks[blocks.length - 1].body, extractTimes(blocks[blocks.length - 1].body))
  ...
  return {
    meta: {
      ...
      flight_in: lastCode,
      flight_out_time: firstTimes.depTime,
      flight_in_time: lastTimes.depTime,
    },
    days,
    confidence,
  };
