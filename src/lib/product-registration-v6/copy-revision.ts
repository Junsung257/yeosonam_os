...
const MINIMUM_CUSTOMER_COPY_SCORE = 72;
...
function copyQualityScore(copy: ProductRegistrationCustomerCopy): number {
  let score = 0;
  ...
  if (copy.reasons.length === 3 && new Set(copy.reasons).size === 3
    && copy.reasons.every(reason => reason.length >= 10 && reason.length <= 100)) score += 25;
...
export function validateProductRegistrationCustomerCopy(input: {
  ...
  if (copyQualityScore(input.copy) < MINIMUM_CUSTOMER_COPY_SCORE) failures.add('CUSTOMER_COPY_QUALITY_BELOW_MINIMUM');
  return [...failures];
}
...
function buildDeterministicFacts(input: {
  ...
  return {
    ...
    departure_dates: firstList(input.pkg, ['departure_dates', 'available_dates'], 12),
...
function buildDeterministicCustomerCopy(facts: JsonObject, degradedReasons: string[]): ProductRegistrationCustomerCopy {
  ...
  const reasons = [
    ...highlights,
    '출발일과 판매 조건을 한 화면에서 비교할 수 있습니다.',
    '포함·불포함 사항과 추가 비용 조건을 예약 전에 확인할 수 있습니다.',
    '예약 전 현재 좌석과 최종 요금을 다시 확인해 드립니다.',
  ].slice(0, 3);
...
function buildCopyPayload(input: {
  ...
  return {
    ...
    copy_policy_version: PRODUCT_REGISTRATION_COPY_POLICY_V2,
...
export function buildProductRegistrationV6Copy(input: {
  ...
  const qualityScore = copyQualityScore(copy);
...
export function createLedgeredProductRegistrationCopyCaller(input: {
  ...
  return async (request: { deterministicFactsHash: string; promptHash: string; systemPrompt: string; userPrompt: string }) => {
    ...
    const requestHash = sha256Hex(stableJson({
      version: PRODUCT_REGISTRATION_COPY_POLICY_V2,
      model,
    ...
    const operationKey = `customer-copy:v2:${input.revisionId}:${request.deterministicFactsHash}:${request.promptHash}`;
    const reservationPayload = {
      ...
      source_hash: input.sourceHash,
      revision_hash: input.revisionHash,
      created_version: PRODUCT_REGISTRATION_COPY_POLICY_V2,
    };
...
export async function generateProductRegistrationV6Copy(input: {
  ...
  const { data: cached, error: cacheError } = await input.supabase.rpc('get_product_registration_v6_cached_copy', {
    ...
    p_locale: 'ko-KR',
    p_copy_policy_version: PRODUCT_REGISTRATION_COPY_POLICY_V2,
  ...
  const qualityScore = copyQualityScore(candidate);
  const payload = buildCopyPayload({
    copy: candidate,
    facts,
    deterministicFactsHash: deterministic.deterministicFactsHash,
