...
import {
  ...
  PRODUCT_REGISTRATION_COPY_POLICY_V3,
...
describe('V6 copy revision', () => {
  ...
  it('keeps facts and presentation policy separate', () => {
    const result = buildProductRegistrationV6Copy({
      pkg: {
        ...
        price_dates: [{ date: '2026-10-01', price: 599000 }],
        shopping_count: 1,
        optional_tours: [],
        minimum_departure_pax: 8,
        lodging_state: 'confirmed',
        product_highlights: ['시내 일정', '시내 일정'],
      },
    ...
    expect(result.payload).toMatchObject({
      title: '다낭 4일',
      policy: PRODUCT_REGISTRATION_COPY_POLICY_V3,
  ...
  it('extracts source specificity from structured dates, hotels and itinerary rows', () => {
    const result = buildProductRegistrationV6Copy({
      pkg: {
        title: '부산 출발 다낭 3박 4일',
        destination: '다낭',
        departure_airport: '김해국제공항',
        price_dates: [{ date: '2026-10-01', price: 899000 }],
        shopping_count: 1,
        optional_tours: [],
        minimum_departure_pax: 8,
        lodging_state: 'confirmed',
        hotels: [{ name: '멜리아 빈펄 다낭' }],
        itinerary_highlights: [{ title: '호이안 올드타운 야경 산책' }],
        inclusions: ['왕복 항공권'],
      },
      claims: [],
      degradedReasons: [],
    });
    expect(result.blockers).not.toContain('CUSTOMER_COPY_DEPARTURE_DATES_MISSING');
    expect(result.payload.reasons).toEqual(expect.arrayContaining([
      expect.stringContaining('멜리아 빈펄 다낭'),
      expect.stringContaining('호이안 올드타운'),
    ]));
  });
