...
function privacyScopedInput(input: MetaConversionInput): MetaConversionInput {
  if (input.consentGranted) return input;
  return {
    ...input,
    fbp: null,
    fbc: null,
...
export async function sendMetaConversion(input: MetaConversionInput) {
  ...
  const scopedInput = privacyScopedInput(input);
  const payload = buildMetaPayload(scopedInput);

  if (!input.consentGranted) {
    await logMetaConversion(scopedInput, {
      sent_to_meta: false,
