...
function classifyClaim(segment: string): Pick<ExtractedBlogInformationClaim, 'claimType' | 'riskLevel' | 'candidateKind'> | null {
  ...
  // Reviewed research packets may contain stable, non-numeric facts such as
  // a guide's location or facility description. Recognize the source-shaped
  // sentence as factual so the rewrite packet can carry it forward. This is
  // still fail-closed at publication: an unlisted sentence has no ledger
  // fingerprint and remains an unclassified candidate.
  if (SOURCED_FACTUAL_ASSERTION_RE.test(segment)) {
    return { claimType: 'factual', riskLevel: 'MEDIUM', candidateKind: 'unknown_statement' };
  }
  return null;
}
...
// A normal-period/current-forecast caveat is reader guidance, not a new
// destination fact. Keep the allowlist narrow: it must name a baseline and
// current visit/forecast variability, and it may not contain a number or a
// regulated/operational assertion.
const V3_SOURCE_NEUTRAL_WEATHER_VARIABILITY_RE = /^(?!.*(?:\d|[₩￦¥￥$€₫]|\b(?:JPY|KRW|USD|VND|SGD|CNY|EUR|THB)\b))(?=.*(?:평년|기후값|기준))(?=.*(?:실제|방문\s*시점|출발\s*시점))(?=.*(?:날씨|예보|조건))(?=.*(?:달라질|변동|다를)).+$/i;
...
function isSourceNeutralPlanningAdvice(
  ...
  return V3_SOURCE_NEUTRAL_PLANNING_IMPERATIVE_RE.test(segment)
    ...
    || V3_SOURCE_NEUTRAL_PRICE_FRESHNESS_RE.test(segment)
    || V3_SOURCE_NEUTRAL_WEATHER_VARIABILITY_RE.test(segment);
}
