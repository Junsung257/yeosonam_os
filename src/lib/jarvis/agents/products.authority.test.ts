...
describe('Jarvis products agent authority boundary', () => {
  it('does not read legacy travel_packages for product search, detail, recommendation, or pairwise comparison', () => {
    ...
    expect(source).toContain('getCustomerCatalogIdSet');
    expect(source).toContain('catalogIds.has(fact.packageId)');
    ...
    expect(source).not.toContain('snapshot_hash: fact.snapshotHash');
    expect(source).not.toContain('entity_relations: aFact.entityRelations');
  });
});
