...
describe('public catalog database contract', () => {
  ...
  it('raises the catalog boundary to grounded copy and documentary media readiness', () => {
    expect(readinessMigration).toContain("product-registration-customer-copy-v3");
    expect(readinessMigration).toContain('quality_score');
    expect(readinessMigration).toContain("media_readiness_state' = 'verified_documentary'");
    expect(readinessMigration).toContain('catalog_generation_id');
    expect(readinessMigration).toContain('minimum_departure_pax');
    expect(readinessMigration).toContain('mandatory_local_costs');
    expect(readinessMigration).toContain('itinerary_intensity');
  });
});
