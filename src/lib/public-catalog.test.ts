...
const row = {
  ...
  pointer_version: 4,
  catalog_generation_id: 'catalog-generation-4',
  ...
  companion_fit: '부모님과 함께 핵심 일정을 보고 싶은 여행자',
  copy_quality_score: 92,
...
describe('public catalog', () => {
  ...
  it('fails closed for expired dates, generic copy, logos and unverified media', async () => {
    const query = queryResult([
      { ...row, id: 'expired', available_dates: [{ date: '2020-01-01', price: 1 }] },
      { ...row, id: 'weak-copy', copy_quality_score: 81 },
      { ...row, id: 'logo', hero_image: '/logo.png' },
      { ...row, id: 'unverified-media', media_readiness_state: 'reference_only' },
    ]);
    const supabase = { from: vi.fn(() => query) };
    await expect(listPublicCatalog(supabase as never)).resolves.toEqual([]);
  });
