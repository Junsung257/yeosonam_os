...
export function toJarvisPublishedPackage(fact: PublishedProductFact): JsonObject {
  ...
  return {
    ...
    departure_airport: card.departure_airport ?? lp.departure_airport ?? null,
    duration: card.duration ?? lp.duration ?? null,
    nights: card.nights ?? lp.nights ?? null,
    product_type: card.product_type ?? lp.product_type ?? null,
    product_summary: card.product_summary ?? lp.product_summary ?? lp.summary ?? null,
    product_highlights: card.product_highlights ?? lp.product_highlights ?? [],
    inclusions: lp.inclusions ?? [],
    excludes: lp.excludes ?? [],
    itinerary: lp.itinerary ?? lp.days ?? [],
    price_dates: priceDates,
    last_verified_at: fact.asOfDate ?? null,
  };
}
