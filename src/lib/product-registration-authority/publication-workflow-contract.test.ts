...
describe('product registration publication workflow contract', () => {
  ...
  it('starts the durable workflow only from the server-attributed admin request', () => {
    ...
    expect(requestRoute).toContain(".eq('channel', 'customer')");
    expect(requestRoute).not.toContain(".in('channel', ['customer', 'b2b', 'partner'])");
    expect(source('src/lib/product-registration-authority/publication.ts')).toContain("channels: ['customer']");
  });
});
