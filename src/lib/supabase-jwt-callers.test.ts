...
describe('pinned Supabase JWT caller regression', () => {
  ...
  it('keeps the tenant portal platform-admin path compatible with the pinned verifier', async () => {
    const token = await adminAccessToken();
    const request = new NextRequest(
      `https://www.yeosonam.com/api/tenant/products?tenant_id=${TENANT_ID}`,
      { headers: { cookie: `sb-access-token=${token}` } },
    );

    await expect(requireTenantPortalRequest(request, TENANT_ID)).resolves.toMatchObject({
      tenantId: TENANT_ID,
      userId: USER_ID,
      role: 'platform_admin',
