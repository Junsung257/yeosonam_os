...
function isOptionHeadingOrNonCustomerOption(text: string): boolean {
  ...
  const withoutExclusionHeading = compact.replace(
    /^(?:불포함(?:내역|사항)?|불포함사항|제외(?:사항)?)[：:]?/u,
    '',
  );
  ...
  const policyParts = withoutExclusionHeading.split(/[,，/|·&]/u).map(part => part.trim()).filter(Boolean);
  if (policyParts.length >= 1
    && policyParts.every(part => /^(?:선택관광(?:비용)?|쇼핑|옵션(?:투어)?)$/u.test(part))) return true;
  if (/^(?:불포함(?:내역|사항)?|불포함사항|제외(?:사항)?)[：:]?/u.test(compact)
    && /(?:선택관광비용|옵션비용)/u.test(compact)
    && !USD_RE.test(normalized)) return true;
