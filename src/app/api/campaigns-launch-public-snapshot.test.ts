...
describe('campaign launch public package boundary', () => {
  it('requires approved public package snapshots before launching customer-facing ads', () => {
    ...
    const publicPackageIndex = source.indexOf('const publicPackagesById = await loadPublicPackagesForCampaignLaunch');
    const launchLoopIndex = source.indexOf('for (const creative of launchableCreatives)');

    expect(source).toContain('loadPublicPackagesForCampaignLaunch');
    expect(source).toContain('fetchAndMergeCurrentPublicPackageCardSnapshots');
    expect(source).toContain('isCampaignPublicSnapshotCandidate');
  ...
  it('does not join raw travel_packages customer fields in the launch query', () => {
    ...
    expect(postSource).toContain('.select(CAMPAIGN_LAUNCH_CREATIVE_FIELDS)');
    expect(postSource).toContain('travel_packages: publicPackagesById.get(productId) ?? null');
  });
});
