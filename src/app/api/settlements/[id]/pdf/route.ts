...
export async function GET(request: NextRequest, props: { params: Promise<{ id: string }> }) {
    ...
    if (!pinAffiliateId) {
      ...
      if (!code || !/^\d{4}$/.test(pin)) {
        return NextResponse.json(
          { error: '어드민 로그인 또는 파트너 인증이 필요합니다.' },
          { status: 401 },
        );
      }
  ...
  const pdfDoc = React.createElement(SettlementPdfDocument, {
    ...
    bookings: qualifiedBookings.map((b: any) => ({
      package_title: b.package_title,
      pax: (b.adult_count || 0) + (b.child_count || 0),
      base_amount: (b.adult_count || 0) * (b.adult_price || 0) + (b.child_count || 0) * (b.child_price || 0),
      commission: b.influencer_commission || 0,
