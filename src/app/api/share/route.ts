...
export async function POST(request: NextRequest) {
  ...
  if (!share_type || !['DYNAMIC', 'FIXED'].includes(share_type)) {
    return apiResponse({ error: 'share_type 필수 (DYNAMIC|FIXED)' }, { status: 400 });
  }
  if (share_type === 'DYNAMIC' && (!items || !items.length)) {
    return apiResponse({ error: 'DYNAMIC 공유는 items 필수' }, { status: 400 });
  }
  if (share_type === 'FIXED' && !product_id) {
    return apiResponse({ error: 'FIXED 공유는 product_id 필수' }, { status: 400 });
  }
  ...
  if (!shared) {
    return apiResponse({ error: '공유 링크 생성 실패' }, { status: 500 });
  }

  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL ?? '';
  const share_url = `${baseUrl}/share/${shared.share_code}`;

  return apiResponse({ share_code: shared.share_code, share_url });
}

export async function GET(request: NextRequest) {
  const code = request.nextUrl.searchParams.get('code');
  if (!code) {
    return apiResponse({ error: 'code 파라미터 필수' }, { status: 400 });
  }
  if (!isSupabaseConfigured) {
    return apiResponse({ error: 'Supabase 미설정' }, { status: 503 });
  }
  ...
  if (!shared) {
    return apiResponse({ error: '존재하지 않거나 만료된 링크입니다.' }, { status: 404 });
  }

  return apiResponse({ shared }, { headers: cacheHeader(600) });
}
