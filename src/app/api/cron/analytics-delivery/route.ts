...
import { processAnalyticsEventOutbox } from '@/lib/analytics/event-outbox';
...
const handler = async (_request: NextRequest) => {
  try {
    const outbox = await processAnalyticsEventOutbox(50);
    const delivery = await processGa4DeliveryJobs(20);
    return apiResponse({ ok: true, outbox, delivery });
  } catch (error) {
