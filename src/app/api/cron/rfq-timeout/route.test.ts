...
describe('RFQ timeout cron service repository', () => {
  ...
  it('processes expired bids only through the service-role RFQ repository', async () => {
    ...
    vi.mocked(updateRfqBid).mockResolvedValue(undefined);
    ...
    expect(updateRfqBid).toHaveBeenCalledWith('bid-1', expect.objectContaining({ status: 'timeout', is_penalized: true }));
