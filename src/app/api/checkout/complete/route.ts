...
async function readVerifiedPaymentConfirmation(
  ...
  let query = sb
    ...
    .select('id, transaction_id, provider, provider_payment_id, provider_order_id, amount_krw, currency, status, verified_at')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const succeeded = bookResults
      .filter((result): result is PromiseFulfilledResult<{ order: ApiOrder; external_ref: string }> => result.status === 'fulfilled')
    ...
    if (failed.length > 0) {
      ...
      await Promise.allSettled(
        failedOrders.map(order => updateApiOrder(order.id, { status: 'CANCELLED' })),
      );

      sagaLog = addSagaEvent(
        sagaLog,
        'ROLLBACK',
        `failed=${failed.length}; rolled_back=${succeeded.length}`,
      );
