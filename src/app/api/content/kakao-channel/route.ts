...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let product: Parameters<typeof generateKakaoChannelMessage>[0]['product'] | undefined = undefined;
    if (body.product_id) {
      const { data: pkg } = await supabaseAdmin
        .from('travel_packages')
        .select('title, destination, duration, nights, price, product_summary')
    ...
    const msg = await generateKakaoChannelMessage({ brief, product, message_type: body.message_type });
    ...
    const row = {
      ...
      payload: msg,
      status: 'draft',
      generation_agent: 'kakao-channel-v1',
      generation_config: { brief, message_type: body.message_type ?? 'promo' },
      updated_at: now,
    };
    ...
    return NextResponse.json({ distribution_id, message: msg });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[kakao-channel] 실패:', msg);
