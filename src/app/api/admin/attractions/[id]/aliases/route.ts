...
export const POST = withAdminGuard(async (req: NextRequest, ctx?: { params?: Promise<{ id: string }> }) => {
  ...
  if (!alias || alias.length < 2 || alias.length > MAX_ALIAS_LENGTH) {
    return NextResponse.json({ error: `alias 길이는 2~${MAX_ALIAS_LENGTH}자` }, { status: 400 });
  }
  if (!/[가-힣A-Za-z]/.test(alias)) {
    return NextResponse.json({ error: '한글 또는 영문 포함 필수' }, { status: 400 });
  }
  ...
  if (prev.some(a => a.toLowerCase().replace(/\s+/g, '') === key)) {
    return NextResponse.json({ ok: true, aliases: prev, message: '이미 존재' });
  }
  if (prev.length >= MAX_ALIASES) {
    return NextResponse.json({ error: `최대 ${MAX_ALIASES}개 제한` }, { status: 400 });
  }
...
export const DELETE = withAdminGuard(async (req: NextRequest, ctx?: { params?: Promise<{ id: string }> }) => {
  ...
  if (next.length === prev.length) {
    return NextResponse.json({ ok: true, aliases: prev, message: '존재하지 않음' });
  }
  const { error: upErr } = await supabaseAdmin
    .from('attractions')
    .update({ aliases: next })
