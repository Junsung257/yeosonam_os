...
const postHandler = async (request: NextRequest) => {
  ...
  try {
    ...
    const reviewerSlot = body?.reviewerSlot === 'first' || body?.reviewerSlot === 'second'
      ? body.reviewerSlot
      : null;
    const annotation = annotationFromJson(body?.annotation);
    if (!/^[0-9a-f-]{36}$/iu.test(corpusSourceId) || !reviewerSlot || !annotation) {
      return ApiErrors.badRequest('원문, 검수 순서 또는 정답지 형식이 올바르지 않습니다.');
    }
    ...
    console.error('[benchmark-review-submit]', error);
