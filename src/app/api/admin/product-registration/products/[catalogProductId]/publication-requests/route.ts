...
const postHandler = async (request: NextRequest, context?: RouteContext) => {
  ...
  try {
    ...
    const expectedPointerVersions = { customer: 0 };
    for (const row of (pointerData ?? []) as PublicationPointerRow[]) {
      if (row.channel !== 'customer') continue;
    ...
    const result = await requestProductRegistrationPublication({
      ...
      request: {
        ...
        requestReason,
        idempotencyKey: `publish:customer:${productId}:${expectedRevisionId}:${expectedPointerVersions.customer}`,
      },
    });
