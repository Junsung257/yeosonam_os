...
import { createReviewReceipt, type ProductReviewDecision, type ReviewEvidenceRefV1 } from '@/lib/product-registration-v6/human-review';
...
async function submit(request: NextRequest, context: Context | undefined, forcedSlot?: 'adjudicator') {
  ...
  try {
    ...
    if (reviewerSlot !== 'first' && reviewerSlot !== 'second' && reviewerSlot !== 'adjudicator') {
    ...
    const data = await callProductReviewRpc<unknown>(supabase, 'submit_product_registration_review_receipt', {
      p_payload: { receipt },
    });
...
export const POST = withAdminGuard((request, context) => submit(request, context, undefined));
