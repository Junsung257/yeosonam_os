...
const AD_OS_STAGING_VALIDATION_TIMEOUT_MS = 8000;
...
export const GET = withAdminGuard(async () => {
  try {
    const summaryResponse = await withTimeout(
      ...
      AD_OS_STAGING_VALIDATION_TIMEOUT_MS,
      'ad os staging validation',
    );
    const summary = await summaryResponse.json();
    const smoke = buildAdOsStagingSmokeSummary();
    const enterprise = summary?.enterprise_layer || {};
    ...
    const tenantPolicy = summary?.tenant_policy || {};
    ...
    const validation = buildAdOsStagingValidationPackage({
      completionAudit: completion,
      stagingSmoke: smoke,
      operatingInventory,
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        top_blocker: validation.top_blocker,
        next_action: validation.next_action,
      },
      gates: {
        smoke,
        operating_inventory: {
          status: operatingInventory.status,
          readiness_score: operatingInventory.readiness_score,
          blocked: operatingInventory.blocked,
          next_action: operatingInventory.next_action,
        },
        live_spend_preflight: {
          status: preflight.status,
          live_write_allowed: preflight.live_write_allowed,
          blockers: preflight.blockers,
          next_action: preflight.next_action,
        },
        learning_evidence: {
          status: learningEvidence.status,
          readiness_score: learningEvidence.readiness_score,
          facts: learningEvidence.facts,
          next_action: learningEvidence.next_action,
        },
      },
    });
  } catch (error) {
    return NextResponse.json(
      {
        ok: false,
        status: 'blocked',
        error: error instanceof Error ? error.message : 'staging validation unavailable',
        next_action: 'Recover /api/admin/ad-os/summary before running the staging validation package.',
        safety: {
          read_only: true,
