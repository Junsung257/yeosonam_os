...
export const GET = withAdminGuard(async (request: NextRequest) => {
  try {
    ...
    const result = evaluateLiveSpendPreflight({
      ...
      operating_inventory_blocked: Number(enterprise.operating_inventory?.blocked || 0),
      staging_smoke_passed: smoke.status === 'pass',
      external_write_count: Number(enterprise.operating_inventory?.external_api_write_count || 0) + Number(enterprise.runtime_execution?.external_api_write_count || 0),
