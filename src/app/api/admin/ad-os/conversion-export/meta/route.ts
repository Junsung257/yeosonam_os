...
import { buildMetaConversionExportPackets, summarizeConversionPackets } from '@/lib/ad-os-v31-v40';
...
export const POST = withAdminGuard(async (request: NextRequest) => {
  ...
  const { data: run, error: runError } = await supabaseAdmin
    ...
    .insert({
      ...
      status: 'running',
      summary: { platform: 'meta', apply, limit, external_api_write: false },
    })
    .select('id')
    .single();
  ...
  const { data: events, error } = await supabaseAdmin
    ...
    .select('*')
    .in('event_type', ['click', 'landing_view', 'cta_click', 'lead', 'booking', 'revenue', 'margin', 'settlement_confirmed'])
  ...
  const packets = buildMetaConversionExportPackets(events || []);
  ...
  if (apply && readyPackets.length > 0) {
    const { error: requestError } = await supabaseAdmin.from('ad_os_change_requests').insert(
      readyPackets.map((packet) => ({
        ...
        status: 'proposed',
        title: 'Meta CAPI 전환 업로드 승인 후보',
        reason: 'Meta Conversions API 업로드 가능한 clean 전환 신호입니다.',
        risk_level: 'medium',
        expected_impact: json({ value_krw: packet.value_krw, margin_krw: packet.margin_krw }),
        proposed_change: json({ platform_upload: 'meta', packet, external_api_write: false }),
  ...
  await supabaseAdmin
    ...
    .update({
      ...
      summary: { ...summary, platform: 'meta', change_requests_created: apply ? readyPackets.length : 0, external_api_write: false },
    })
    .eq('id', run.id);
