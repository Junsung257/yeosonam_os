...
export const GET = withAdminGuard(async (request: NextRequest): Promise<NextResponse> => {
  try {
    ...
    return apiResponse({ ok: true, plans, summary });
  } catch (err) {
    return apiResponse(
      { ok: false, error: sanitizeDbError(err, 'Failed to load search ad plans') },
      { status: 500 },
    );
  }
});
...
const handler = async (request: NextRequest): Promise<NextResponse> => {
  try {
    ...
    if (body.package_id) {
      ...
      return apiResponse({ ok: true, saved: plan.saved, plan });
    }

    if (body.package?.id) {
      const plan = await buildSearchAdPackagePlan(body.package);
      return apiResponse({ ok: true, saved: 0, plan });
    }

    return apiResponse(
      { ok: false, error: 'package_id or package body is required' },
      { status: 400 },
    );
  } catch (err) {
    return apiResponse(
      { ok: false, error: sanitizeDbError(err, 'Failed to build search ad plan') },
      { status: 500 },
    );
  }
};
