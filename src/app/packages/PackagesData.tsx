...
export default async function PackagesData({ searchParams }: Props) {
  const params = await searchParams;
  const destination = params.destination || '';
  const rawFilter = params.filter || '';
  /** 레거시 칩 "인천출발" 북마크 → 출발 허브 인천 (허브 필터로 이관) */
  let hub = normalizeDepartureHub(params.hub);
  if (rawFilter === '인천출발' && !params.hub) hub = 'incheon';
