...
export default function PackagesListPage() {
  ...
  const [activeFilter, setActiveFilter] = useState(filterParam || '전체');
  ...
  useEffect(() => {
    async function load() {
      try {
        const url = destParam
          ? `/api/packages?status=approved&destination=${encodeURIComponent(destParam)}&limit=50`
          : '/api/packages?status=approved&limit=50';
        // 1단계: 상품 목록 먼저 표시
        const res = await fetch(url);
        const data = await res.json();
        let pkgs = data.data ?? data.packages ?? [];
        if (destParam) {
          pkgs = pkgs.filter((p: any) => p.destination === destParam || p.destination?.includes(destParam) || destParam.includes(p.destination || ''));
        }
        setPackages(pkgs);
        setLoading(false);

        // 2단계: 관광지 사진 비동기 로드 (UI 블로킹 없음)
        fetch('/api/attractions?photos_only=1')
          .then(r => r.json())
