...
export default function ContractPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/rfq/${encodedRfqId}/contract`)
      .then(r => {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4">
        ...
        <Link href={`/rfq/${encodedRfqId}`} className="text-brand text-sm hover:underline">
          ← 견적 현황으로
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-10 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href={`/rfq/${encodedRfqId}`} className="text-gray-400 hover:text-gray-600 text-sm">←</Link>
            <div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-12">
        <div className="bg-brand-light border border-blue-200 rounded-xl p-5 text-center">
          <h3 className="font-semibold text-text-primary mb-2">계약 동의 및 결제</h3>
          <p className="text-sm text-brand mb-4">
            위 계약 내용에 동의하고 에스크로 결제를 진행하시면 계약이 확정됩니다.
          </p>
          <div className="flex gap-3 justify-center">
            <Link
              href={`/rfq/${encodedRfqId}`}
