    ...
    return (
      <div className="min-h-screen bg-admin-bg py-8">
        <div className="max-w-3xl mx-auto px-4 text-center py-20">
          <p className="text-admin-muted">예약을 찾을 수 없습니다.</p>
    ...
    <div className="min-h-screen bg-admin-bg py-8">
      <div className="max-w-3xl mx-auto px-4">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="font-mono text-xs text-admin-muted-2">{booking.booking_no || id.slice(0, 8)}</span>
        </div>

        <h1 className="text-2xl font-bold text-admin-text mb-6">예약 수정</h1>

        <form onSubmit={handleSubmit} className="space-y-6">
          {/* 상태 */}
          <div className="bg-white rounded-admin-md shadow-admin-xs p-5">
            <h2 className="font-semibold text-admin-text mb-4">예약 상태</h2>
          ...
          {/* 상품 선택 */}
          <div className="bg-white rounded-admin-md shadow-admin-xs p-5">
            <h2 className="font-semibold text-admin-text mb-4">상품</h2>
              ...
              {packages.map(pkg => (
                <button key={pkg.id} type="button"
                  ...
                  className={`text-left p-3 rounded-lg border transition ${
                    form.packageId === pkg.id ? 'border-blue-500 bg-blue-50' : 'border-admin-border-mid hover:border-admin-border-strong'
                  }`}>
                  <p className="font-medium text-admin-text text-sm">{pkg.title}</p>
                  <p className="text-xs text-admin-muted">{pkg.destination} {pkg.price && `· ${pkg.price.toLocaleString()}원`}</p>
                </button>
              ))}
            </div>
            {form.packageId ? (
          ...
          {/* 대표 예약자 */}
          <div className="bg-white rounded-admin-md shadow-admin-xs p-5">
            <h2 className="font-semibold text-admin-text mb-4">대표 예약자</h2>
              ...
              <div className="flex items-center justify-between gap-2 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                ...
                <div className="flex gap-1.5 shrink-0">
                  ...
                  <button type="button"
                    ...
                    className="text-admin-xs text-admin-text-2 border border-admin-border-mid bg-white px-2.5 py-1 rounded-md hover:bg-admin-bg transition">
                    다른 고객으로 변경
                  </button>
                </div>
              </div>
              ...
              <div className="p-3 border border-amber-200 bg-amber-50 rounded-lg space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input type="text" value={editCustomerName}
                    ...
                    placeholder="이름"
                    className="border border-admin-border-strong rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" />
                ...
                <div className="flex gap-1.5 justify-end">
                  <button type="button" onClick={() => setCustomerMode('view')}
                    className="text-admin-xs text-admin-muted px-3 py-1.5 rounded-md hover:bg-admin-surface-2">취소</button>
              ...
              <div className="space-y-2">
                ...
                {currentLeadCustomer && (
                  <button type="button" onClick={() => { setCustomerSearch(currentLeadCustomer.name); setCustomerMode('view'); }}
                    className="text-[11px] text-admin-muted hover:text-admin-text-2 underline">
          ...
          {/* 동행자 */}
          <div className="bg-white rounded-admin-md shadow-admin-xs p-5">
            <h2 className="font-semibold text-admin-text mb-4">동행자</h2>
            <input
              type="text"
            ...
            {filteredPassengers.length > 0 && (
              <div className="border border-admin-border-mid rounded-lg overflow-hidden mb-2">
                ...
                {selectedPassengers.map(p => (
                  <span key={p.id} className={`flex items-center gap-1 text-xs px-3 py-1 rounded-full ${
                    ...
                    <button type="button" onClick={() => setSelectedPassengers(prev => prev.filter(x => x.id !== p.id))}
                      className="text-admin-muted-2 hover:text-admin-muted ml-1">×</button>
                  </span>
                ))}
              </div>
            )}
          ...
          <div className="bg-white rounded-admin-md shadow-admin-xs p-5">
            ...
            <div className="mt-4 p-4 bg-admin-bg rounded-admin-md border border-admin-border shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
              ...
              <div className="flex justify-between text-sm pt-2 border-t border-admin-border-mid mt-1">
                <span className="text-admin-text-2 font-medium">마진</span>
                <div className="text-right">
                  <span className={`font-bold ${margin >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {margin.toLocaleString()}원
                  </span>
                  {totalPrice > 0 && (
                    <span className="text-xs text-admin-muted-2 ml-2">({marginRate}%)</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => router.push(`/admin/bookings/${id}`)}
              className="flex-1 text-center bg-admin-surface-2 text-admin-text-2 py-3 rounded-admin-md text-sm hover:bg-slate-200 transition">
              상세 보기로
            </button>
          </div>
        </form>
