...
export default function LandOperatorsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-[16px] font-semibold text-slate-800">랜드사 관리</h1>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-4">
        <h2 className="text-[14px] font-semibold text-slate-800 mb-3">신규 랜드사 추가</h2>
        <div className="flex gap-2">
          ...
          <input
            ...
            className="w-52 border border-slate-200 rounded px-3 py-2 text-[13px] text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
          <button
            onClick={handleAdd}
      ...
      {loading ? (
        ...
        <p className="text-slate-500 text-[13px]">등록된 랜드사가 없습니다.</p>
      ) : (
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          <table className="w-full text-[13px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-3 py-2 text-left text-[11px] font-medium text-slate-500">랜드사명</th>
                <th className="px-3 py-2 text-left text-[11px] font-medium text-slate-500">연락처</th>
                  ...
                  <td className="px-3 py-2 font-medium text-slate-800">
                    {editingId === v.id ? (
                      <input
                        ...
                        className="border border-blue-400 rounded px-2 py-1 text-[13px] w-full focus:outline-none focus:ring-1 focus:ring-blue-500"
                      />
                    ) : (
                      <button
                        onClick={() => startEdit(v)}
                    ...
                    {editingId === v.id ? (
                      <div className="flex gap-1">
                        <input
                          ...
                          placeholder="연락처"
                          className="border border-blue-400 rounded px-2 py-1 text-[13px] w-32 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <button
