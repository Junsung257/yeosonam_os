...
export function ActionCard({ action, onApprove, onReject }: ActionCardProps) {
  ...
  return (
    <div className={`rounded-admin-md border-2 ${style.border} ${style.bg} p-4 my-3 max-w-md`}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-admin-text-2">자비스 실행 요청</span>
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${style.badge}`}>
          위험도 {style.label}
        </span>
      </div>

      <p className="text-sm font-medium text-admin-text mb-2">{action.description}</p>

      {/* 실행 파라미터 요약 */}
      <div className="bg-white/70 rounded-lg p-2.5 mb-3 text-xs text-admin-muted font-mono">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={loading !== null}
          className="flex-1 py-2 rounded-lg bg-white border border-admin-border-strong text-admin-text-2 text-sm font-semibold hover:bg-admin-bg disabled:opacity-50 transition"
        >
