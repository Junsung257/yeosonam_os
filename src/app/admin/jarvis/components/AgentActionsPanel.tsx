  ...
  return (
    <div className="space-y-3">
      ...
      {actions.length === 0 && !loading ? (
        <div className="text-center py-12 text-admin-muted-2 text-admin-sm">
        ...
        <div className="space-y-2">
          {actions.map(action => (
            <div
              key={action.id}
              className="border border-admin-border-mid rounded-lg bg-white hover:border-admin-border-strong transition"
            >
              {/* 카드 헤더 */}
              <div
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                  ...
                  <span className="ml-auto text-[10px] text-admin-muted-2">
              ...
              {expandedId === action.id && (
                <div className="px-3 pb-3 space-y-2">
                  <div className="bg-admin-bg rounded-lg p-2.5 text-xs text-admin-muted font-mono max-h-60 overflow-y-auto">
                    ...
                    {Object.keys(action.payload).length === 0 && (
                      <span className="text-admin-muted-2">페이로드 없음</span>
                    )}
                  </div>
                    ...
                    <div className="space-y-2">
                      {rejectId === action.id && (
                        <textarea
                          ...
                          className="w-full border border-admin-border-mid rounded px-2 py-1.5 text-admin-xs focus:ring-1 focus:ring-[#005d90] focus:outline-none resize-none"
                          rows={2}
                        />
                      )}
                      <div className="flex gap-1.5">
                        ...
                        {rejectId === action.id ? (
                          ...
                          <button
                            ...
                            className="flex-1 bg-white border border-admin-border-strong text-admin-muted py-1.5 rounded text-[11px] font-medium hover:bg-admin-bg transition"
                          >
                            반려
                          </button>
                        )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button
            ...
            disabled={page <= 1}
            className="px-2 py-1 text-[11px] border border-admin-border-mid rounded hover:bg-admin-bg disabled:opacity-40"
          >
            이전
          </button>
      ...
      {/* 토스트 */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-2.5 rounded-lg text-white text-admin-sm shadow-admin-md ${
