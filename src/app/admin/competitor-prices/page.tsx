...
export default function CompetitorPricesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-admin-text">경쟁사 가격 비교</h1>
      ...
      <div className="bg-admin-surface rounded-admin-md border border-admin-border-mid shadow-admin-xs p-5">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <div>
            <label htmlFor="competitor-price-competitor" className="block text-xs font-medium text-admin-muted mb-1">경쟁사 *</label>
            <select
              id="competitor-price-competitor"
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-admin-border">
                        <th className="text-left px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">경쟁사</th>
                        <th className="text-right px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">가격</th>
                        {yeosonamMin != null && (
                          <th className="text-right px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">
                            여소남 대비
                          </th>
                        )}
                        <th className="text-left px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">출발일</th>
                        <th className="text-left px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">입력일</th>
                        <th className="text-left px-4 py-2.5 text-xs font-medium text-admin-muted uppercase">출처</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map((row) => {
                        ...
                        return (
                          <tr key={row.id} className="border-b border-admin-border hover:bg-admin-bg">
                            <td className="px-4 py-3 font-medium text-admin-text-2">{row.competitor}</td>
                            <td className="px-4 py-3 text-right font-mono font-semibold text-admin-text">
                              {fmt만(row.price)}
                            </td>
                            {yeosonamMin != null && (
                            ...
                            <td className="px-4 py-3">
                              {row.source_url ? (
                                ...
                                <span className="text-admin-muted-2 text-xs">—</span>
                              )}
                            </td>
                          </tr>
                        );
