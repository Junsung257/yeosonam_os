...
const STATUS_BADGE: Record<string, string> = {
  ...
  PUBLISHED:  'bg-green-100 text-green-700',
  idle:       'bg-admin-surface-2 text-admin-muted-2',
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        ...
        <p className="text-sm text-admin-muted">{isNew ? '콘텐츠 생성 중...' : '로딩 중'}</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-admin-bg">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[cn.status] ?? 'bg-admin-surface-2 text-admin-muted'}`}>
            {cn.status}
          </span>
        </div>
      </div>
      ...
      {job && (
        <div className="bg-white border-b px-6 py-3">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs text-admin-muted">파이프라인 진행 ({job.completed_steps}/{job.total_steps})</span>
          ...
          <div className="flex gap-4 mt-2">
            {Object.entries(job.steps).map(([key, step]) => (
              <div key={key} className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-admin-muted-2">{key.replace('_', ' ')}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex h-[calc(100vh-120px)]">
        ...
        <div className="w-64 bg-white border-r p-3 overflow-y-auto flex-shrink-0">
          <p className="text-xs font-medium text-admin-muted mb-2">슬라이드 ({cn.slide_count}장)</p>
          {allRenders1x1.length > 0 ? (
            <div className="grid grid-cols-2 gap-1.5">
              {allRenders1x1.map(r => (
                <div key={r.slide_index} className="relative group">
                  <img src={r.url} alt={`슬라이드 ${r.slide_index + 1}`}
                    className="w-full aspect-square object-cover rounded border border-admin-border" />
            ...
            <div className="text-center py-8 text-admin-muted-2">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-white border-b flex gap-0">
            {tabs.map(t => (
              ...
              >
                {t.label}
                {t.badge && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${STATUS_BADGE[t.badge] ?? 'bg-admin-surface-2 text-admin-muted'}`}>
                    {t.badge}
                  </span>
                )}
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {activeTab === 'card_news' && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <h2 className="font-medium text-admin-text">카드뉴스</h2>
                  <div className="flex gap-2">
                    <a
                      href={`/admin/marketing/card-news/${cn.id}/v2`}
                      className="text-xs px-3 py-1.5 border rounded-lg text-admin-muted hover:bg-admin-bg"
                    >
                      V2 Studio →
                    </a>
                    <a
                ...
                {coverRender ? (
                  <img src={coverRender.url} alt="커버"
                    className="rounded-admin-md border max-w-xs shadow-admin-xs" />
                ) : (
                  <div className="w-48 h-48 bg-admin-surface-2 rounded-admin-md flex items-center justify-center text-admin-muted-2 text-sm animate-pulse">
                    렌더링 중
                  </div>
                )}

                <div className="grid grid-cols-3 gap-3 text-sm text-admin-muted">
                  <div className="bg-admin-bg rounded-lg p-3">
                    <p className="text-xs text-admin-muted-2 mb-1">슬라이드</p>
                    <p className="font-semibold">{cn.slide_count}장</p>
                  </div>
                  <div className="bg-admin-bg rounded-lg p-3">
                    <p className="text-xs text-admin-muted-2 mb-1">렌더 수</p>
                    <p className="font-semibold">{cn.renders.length}개</p>
                  </div>
                  <div className="bg-admin-bg rounded-lg p-3">
                    <p className="text-xs text-admin-muted-2 mb-1">템플릿</p>
                ...
                {/* 비율별 포맷 다운로드 */}
                <div>
                  <p className="text-xs font-medium text-admin-muted mb-2">포맷별 렌더</p>
                  {(['1x1', '4x5', '9x16', 'blog'] as const).map(fmt => {
                    ...
                    return (
                      <div key={fmt} className="flex items-center justify-between py-2 border-b last:border-0">
                        <span className="text-sm text-admin-text-2">{fmt}</span>
                        {fmtRenders.length > 0 ? (
                          <span className="text-xs text-green-600 font-medium">{fmtRenders.length}장 완료</span>
                        ) : (
                          <span className="text-xs text-admin-muted-2">미생성</span>
                        )}
                      </div>
                    );
                  })}
            ...
            {activeTab === 'instagram' && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <h2 className="font-medium text-admin-text">인스타그램</h2>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_BADGE[ig.status] ?? 'bg-admin-surface-2'}`}>
                    {ig.status}
                  </span>
                </div>
            ...
            {activeTab === 'blog' && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <h2 className="font-medium text-admin-text">블로그</h2>
                  {blog && (
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_BADGE[blog.status ?? ''] ?? 'bg-admin-surface-2'}`}>
                      {blog.status}
                    </span>
                  )}
                </div>
                ...
                {blog ? (
                  <div className="space-y-3">
                    <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                      <p className="font-medium text-admin-text text-sm">{blog.title}</p>
                      {blog.url && (
                    ...
                    <div className="flex gap-2">
                      <a
                        ...
                        className="text-sm px-4 py-2 border rounded-lg text-admin-text-2 hover:bg-admin-bg"
                      >
                        블로그 편집 →
                      </a>
                    </div>
            ...
            {activeTab === 'meta_ads' && (
              <div className="space-y-5">
                <h2 className="font-medium text-admin-text">메타 광고 직접 발행</h2>
                ...
                <div className="bg-admin-bg rounded-admin-md border border-admin-border-mid p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm bg-white resize-none"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label htmlFor="content-hub-meta-cta-button" className="text-[11px] font-semibold text-admin-muted uppercase block mb-1">CTA 버튼</label>
                      <select
                        id="content-hub-meta-cta-button"
                ...
                {meta_ads.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-admin-muted uppercase mb-2">발행 내역</p>
                    <div className="space-y-2">
                      {meta_ads.map(d => (
                        <div key={d.id} className="bg-white border rounded-lg p-4 flex items-center justify-between">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_BADGE[d.status] ?? 'bg-admin-surface-2'}`}>
                            {d.status}
                          </span>
                        </div>
                      ))}
                ...
                {distributions.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-admin-muted uppercase mb-2">전체 채널 현황</p>
                    <div className="space-y-1">
                      {distributions.map(d => (
                        <div key={d.id} className="flex items-center justify-between text-xs py-1.5 border-b last:border-0">
                          <span className="text-admin-muted">{d.platform}</span>
                          <span className={`px-1.5 py-0.5 rounded font-medium ${STATUS_BADGE[d.status] ?? 'bg-admin-surface-2'}`}>
                            {d.status}
                          </span>
                        </div>
                      ))}
