    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-admin-muted">카드뉴스·블로그·인스타에 주입되는 브랜드 토큰·보이스 가이드</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white rounded-admin-md border border-admin-border shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 space-y-3">
              <div className="h-4 bg-admin-surface-2 rounded animate-pulse w-36" />
        ...
        <div className="flex flex-col items-center gap-3 py-16">
          ...
          <p className="text-admin-sm font-medium text-admin-muted">브랜드킷이 없습니다.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {kits.map(kit => (
            <div key={kit.id} className="bg-white rounded-admin-md border border-admin-border-mid p-5 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-mono bg-admin-surface-2 text-admin-muted px-2 py-0.5 rounded">{kit.code}</span>
                  <p className="font-semibold text-admin-text-2 mt-1">{kit.name}</p>
                  {kit.domain && <p className="text-xs text-admin-muted-2">{kit.domain}</p>}
                </div>
                <span className={`text-xs px-2 py-1 rounded-full ${kit.is_active ? 'bg-green-100 text-green-700' : 'bg-admin-surface-2 text-admin-muted'}`}>
                ...
                {COLOR_KEYS.map(k => (
                  kit.colors?.[k] ? (
                    <div key={k} title={`${k}: ${kit.colors[k]}`}
                      ...
                      className="w-6 h-6 rounded-full border border-white shadow-admin-xs" />
                  ) : null
                ))}
              </div>

              <p className="text-xs text-admin-muted-2 line-clamp-2">
                {kit.voice_guide ? `"${kit.voice_guide.slice(0, 100)}${kit.voice_guide.length > 100 ? '…' : ''}"` : '보이스 가이드 없음'}
              </p>

              <div className="flex items-center justify-between">
                <p className="text-[10px] text-admin-muted-2">{kit.updated_at?.slice(0, 10)}</p>
                <button
      ...
      {editing && (
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-white z-50 flex flex-col border-l border-admin-border-mid shadow-admin-lg">
            <div className="px-6 py-4 border-b border-admin-border-mid flex items-center justify-between">
              <div>
                <span className="text-xs font-mono bg-admin-surface-2 text-admin-muted px-2 py-0.5 rounded">{editing.code}</span>
                <h2 className="text-admin-lg font-bold text-admin-text-2 mt-1">{editing.name} 편집</h2>
              </div>
              <button onClick={() => setEditing(null)} className="text-admin-muted-2 hover:text-admin-muted text-xl leading-none">×</button>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {/* 기본 정보 */}
              <section className="space-y-3">
                <h3 className="text-xs font-semibold text-admin-muted uppercase tracking-wider">기본 정보</h3>
                <div>
                  <label className="text-[11px] text-admin-muted block mb-1">이름</label>
                  <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
                    className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] text-admin-muted block mb-1">로고 텍스트</label>
                    <input value={editing.logo_text ?? ''} onChange={e => setEditing({ ...editing, logo_text: e.target.value || null })}
                      placeholder="YEOSONAM" className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-admin-muted block mb-1">도메인</label>
                    <input value={editing.domain ?? ''} onChange={e => setEditing({ ...editing, domain: e.target.value || null })}
                      placeholder="yeosonam.com" className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm" />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-admin-muted block mb-1">로고 URL (이미지)</label>
                  <input value={editing.logo_url ?? ''} onChange={e => setEditing({ ...editing, logo_url: e.target.value || null })}
                    placeholder="https://..." className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm" />
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="is_active" checked={editing.is_active}
                    onChange={e => setEditing({ ...editing, is_active: e.target.checked })}
                    className="rounded" />
                  <label htmlFor="is_active" className="text-sm text-admin-text-2">활성 상태</label>
                </div>
              </section>

              {/* 컬러 팔레트 */}
              <section className="space-y-3">
                <h3 className="text-xs font-semibold text-admin-muted uppercase tracking-wider">컬러 팔레트</h3>
                <div className="grid grid-cols-2 gap-2">
                  {COLOR_KEYS.map(k => (
                    <div key={k} className="flex items-center gap-2">
                      <input type="color" value={editing.colors?.[k] || '#000000'}
                        onChange={e => setColor(k, e.target.value)}
                        className="w-8 h-8 rounded cursor-pointer border border-admin-border-mid" />
                      <div>
                        <p className="text-[11px] font-medium text-admin-muted">{k}</p>
                        <input value={editing.colors?.[k] || ''} onChange={e => setColor(k, e.target.value)}
                          placeholder="#000000" className="text-[11px] font-mono text-admin-muted w-20 border-none outline-none" />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* 폰트 */}
              <section className="space-y-3">
                <h3 className="text-xs font-semibold text-admin-muted uppercase tracking-wider">폰트</h3>
                <div className="grid grid-cols-3 gap-2">
                  {['sans', 'serif', 'mono'].map(k => (
                    <div key={k}>
                      <label className="text-[11px] text-admin-muted block mb-1">{k}</label>
                      <input value={editing.fonts?.[k] || ''} onChange={e => setFont(k, e.target.value)}
                        placeholder="Pretendard" className="w-full border border-admin-border-mid rounded px-2 py-1.5 text-xs" />
                    </div>
                  ))}
                </div>
              </section>
              ...
              {/* 보이스 가이드 */}
              <section className="space-y-3">
                <h3 className="text-xs font-semibold text-admin-muted uppercase tracking-wider">브랜드 보이스 가이드</h3>
                <p className="text-[11px] text-admin-muted-2">카드뉴스·블로그 AI 생성 시 LLM에 주입되는 브랜드 톤앤매너 지침</p>
                <textarea
                  ...
                  className="w-full border border-admin-border-mid rounded px-3 py-2 text-sm h-28 resize-none focus:ring-1 focus:ring-blue-300"
                />
              </section>
              ...
              <section className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-semibold text-admin-muted uppercase tracking-wider">보이스 샘플 (Few-shot)</h3>
                  <button onClick={addVoiceSample}
                    className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                    + 추가
                  </button>
                </div>
                <p className="text-[11px] text-admin-muted-2">AI가 글을 쓸 때 참조할 실제 카피 예시. 많을수록 일관성↑</p>
                {(editing.voice_samples ?? []).length === 0 ? (
                  <p className="text-xs text-admin-muted-2 italic">샘플 없음</p>
                ) : (
                      ...
                      <div key={i} className="border border-admin-border-mid rounded-lg p-3 space-y-2">
                        <div className="flex items-center gap-2">
                          <select value={s.platform} onChange={e => updateVoiceSample(i, 'platform', e.target.value)}
                            className="text-xs border border-admin-border-mid rounded px-2 py-1">
                        ...
                        <textarea value={s.text} onChange={e => updateVoiceSample(i, 'text', e.target.value)}
                          ...
                          className="w-full text-xs border border-admin-border-mid rounded px-2 py-1.5 h-16 resize-none" />
                      </div>
                    ))}
                  </div>
                )}
            ...
            {/* 하단 저장 버튼 */}
            <div className="px-6 py-4 border-t border-admin-border-mid flex items-center gap-3">
              {saveMsg && (
              ...
              <div className="ml-auto flex gap-3">
                <button onClick={() => setEditing(null)}
                  className="px-4 py-2 text-sm text-admin-muted border border-admin-border-mid rounded-lg hover:bg-admin-bg">
                  닫기
                </button>
