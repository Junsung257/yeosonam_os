...
function SystemWeaknessPanel() {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-blue-50 border border-admin-border-mid rounded-admin-md p-4 mb-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h2 className="text-sm font-bold text-admin-text-2">🔍 시스템 약점 자동 발견</h2>
      ...
      {statsLoading ? (
        <p className="text-xs text-admin-muted-2 py-3 text-center">분석 중…</p>
      ) : tab === 'field' ? (
        fieldStats.length === 0 ? <p className="text-xs text-admin-muted-2 py-3 text-center">정정 데이터 누적 시 통계 표시</p>
        : (
          <table className="w-full text-xs">
            <thead><tr className="text-admin-muted border-b border-admin-border-mid">
            ...
            <tbody>
              {fieldStats.slice(0, 10).map(s => (
                <tr key={s.field_path} className="border-b border-admin-border">
                  <td className="py-1.5 font-mono text-admin-text-2">{s.field_path}</td>
                  <td className="text-right font-bold text-admin-text-2">{s.total_count}</td>
                  ...
                  <td className="pl-3 text-admin-muted">{s.most_common_category || '-'}</td>
                  <td className="text-right text-admin-muted-2 text-[10px]">{fmtDateISO(s.last_correction_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        categoryStats.length === 0 ? <p className="text-xs text-admin-muted-2 py-3 text-center">정정 데이터 누적 시 통계 표시</p>
        : (
          <table className="w-full text-xs">
            <thead><tr className="text-admin-muted border-b border-admin-border-mid">
            ...
            <tbody>
              {categoryStats.map(s => (
                <tr key={s.category} className="border-b border-admin-border">
                  <td className="py-1.5 text-admin-text-2">{s.category}</td>
                  <td className="text-right font-bold text-admin-text-2">{s.total_count}</td>
                  <td className="text-right text-red-600">{s.critical_count || ''}</td>
                  <td className="text-right text-emerald-600">{s.applied_total}</td>
                  <td className="pl-3 text-admin-muted">{(s.destinations || []).slice(0, 5).join(', ') || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        destinationStats.length === 0 ? <p className="text-xs text-admin-muted-2 py-3 text-center">정정 데이터 누적 시 통계 표시</p>
        : (
          <table className="w-full text-xs">
            <thead><tr className="text-admin-muted border-b border-admin-border-mid">
            ...
            <tbody>
              {destinationStats.map(s => (
                <tr key={s.destination} className="border-b border-admin-border">
                  <td className="py-1.5 text-admin-text-2">📍 {s.destination}</td>
                  <td className="text-right font-bold text-admin-text-2">{s.total_count}</td>
                  <td className="text-right text-red-600">{s.critical_count || ''}</td>
                  <td className="text-right text-amber-600">{s.high_count || ''}</td>
                  <td className="text-right text-admin-muted">{s.unique_fields}</td>
                  <td className="pl-3 font-mono text-admin-muted text-[10px]">{s.most_common_field || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const SEVERITY_BG: Record<string, string> = {
  ...
  low: 'bg-admin-surface-2 border-admin-border-strong text-admin-muted',
};
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <KpiCard label="활성 정정 (현재 필터)" value={stats.total.toLocaleString()} icon={Brain} />
        <KpiCard label="CRITICAL" value={stats.critical.toLocaleString()} icon={AlertTriangle} tone="negative" />
        <KpiCard label="HIGH" value={stats.high.toLocaleString()} icon={AlertCircle} tone={stats.high > 0 ? 'negative' : 'neutral'} />
        <KpiCard
          label="총 prompt 주입"
      ...
      {/* 필터 */}
      <div className="flex gap-3 mb-4 items-center admin-card p-3">
        <input
          type="text"
          value={filterDestination}
          onChange={e => setFilterDestination(e.target.value)}
          placeholder="목적지 필터 (예: 보홀)"
          className="h-9 text-admin-base border border-admin-border-mid rounded-admin-sm px-3 flex-1 bg-admin-surface text-admin-text focus:outline-none focus:shadow-admin-focus focus:border-brand transition-colors"
        />
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white rounded-admin-md border border-admin-border shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-4 space-y-2">
              <div className="h-3.5 bg-admin-surface-2 rounded animate-pulse w-2/3" />
        ...
        <div className="flex flex-col items-center gap-3 py-14">
          ...
          <p className="text-admin-sm font-medium text-admin-muted">활성 정정 메모리가 없습니다.</p>
          <p className="text-admin-xs text-admin-muted-2">패키지를 인라인 편집하면 자동 적립됩니다.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {items.map(c => (
            <div key={c.id} className={`bg-white border rounded-admin-md p-4 border-admin-border-mid`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {c.category && <span className="px-2 py-0.5 text-[10px] font-mono rounded bg-admin-surface-2 text-admin-muted">{c.category}</span>}
                    {c.destination && <span className="text-xs text-admin-muted">📍 {c.destination}</span>}
                    {c.applied_count > 0 && <span className="text-xs text-emerald-600 font-bold">prompt 주입 {c.applied_count}회</span>}
                    <span className="text-[10px] text-admin-muted-2 ml-auto">{fmtDateISO(c.created_at)}</span>
                  </div>
                  <div className="mt-2 font-mono text-xs text-admin-text-2">field: <span className="text-blue-700">{c.field_path}</span></div>

                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <div className="bg-red-50 border border-red-100 rounded p-2">
                      <div className="text-[10px] text-red-600 font-bold mb-1">❌ 이전 (AI 생성)</div>
                      <div className="text-xs text-admin-text-2 font-mono whitespace-pre-wrap break-words">{renderValue(c.before_value)}</div>
                    </div>
                    <div className="bg-emerald-50 border border-emerald-100 rounded p-2">
                      ...
                      <div className="text-xs text-admin-text-2 font-mono whitespace-pre-wrap break-words">{renderValue(c.after_value)}</div>
                    </div>
                  </div>
                  ...
                  {editingId === c.id ? (
                    <div className="mt-2 space-y-2">
                      <textarea
                        ...
                        className="w-full text-sm border border-admin-border-mid rounded-lg px-3 py-2 min-h-[60px]"
                      />
                    ...
                    <div className="mt-2 bg-blue-50 border border-blue-100 rounded p-2">
                      ...
                      <p className="text-xs text-admin-text-2 mt-1">{c.reflection}</p>
                    </div>
                  ) : (
                    <div className="mt-2 text-xs text-admin-muted-2">교훈 미입력 — "교훈 입력" 으로 추가하면 prompt 효과 ↑</div>
                  )}
                </div>
