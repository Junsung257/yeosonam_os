          ...
          {loading ? (
            <div className="space-y-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="bg-admin-surface rounded-admin-md border border-admin-border-mid shadow-admin-xs p-3 space-y-2">
        ...
        {selectedItem && (
          <div className="w-96 flex-shrink-0 border border-admin-border-mid rounded-lg bg-white p-4 space-y-4 overflow-y-auto max-h-[calc(100vh-200px)]">
            ...
            {tab === 'draft' && (
              <>
                <div>
                  <label htmlFor="content-queue-edit-slug" className="block text-[10px] text-admin-muted-2 mb-1">URL 슬러그</label>
                  <div className="flex items-center gap-1">
                    <span className="text-[11px] text-admin-muted-2">/blog/</span>
                    <input id="content-queue-edit-slug" value={editSlug}
                ...
                <div>
                  <label htmlFor="content-queue-edit-title" className="block text-[10px] text-admin-muted-2 mb-1">SEO 제목</label>
                  <input id="content-queue-edit-title" value={editTitle} onChange={e => setEditTitle(e.target.value)}
                    maxLength={60}
                ...
                <div>
                  <label htmlFor="content-queue-edit-description" className="block text-[10px] text-admin-muted-2 mb-1">SEO 설명</label>
                  <textarea id="content-queue-edit-description" value={editDesc} onChange={e => setEditDesc(e.target.value)}
                    maxLength={160}
                ...
                <div>
                  <label htmlFor="content-queue-edit-og-image" className="block text-[10px] text-admin-muted-2 mb-1">OG 이미지</label>
                  <input id="content-queue-edit-og-image" value={editOgImage} onChange={e => setEditOgImage(e.target.value)}
                    placeholder="https://..."
                    className="w-full border border-admin-border-mid rounded px-2 py-1 text-admin-xs focus:ring-1 focus:ring-indigo-400" />
                </div>
