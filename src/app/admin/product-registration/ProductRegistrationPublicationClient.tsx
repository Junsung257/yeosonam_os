...
function canRequestPublication(row: AdminPackagePublicationTruth): boolean {
  ...
  if (row.customerReadinessBlockerCodes.length > 0) return false;
              ...
              <article key={row.catalogProductId} className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,1fr)_auto] lg:items-center">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-admin-xs text-admin-muted">
                    revision {row.latestRevisionNo ?? '-'} · 미래 출발 {row.futureDepartureCount} · 가격확정 {row.pricedDepartureCount} · 문의가 {row.requestOnlyDepartureCount}
                  </p>
                  <p className="mt-1 text-admin-xs text-admin-muted">
                    고객문구 {row.copyQualityScore ?? '-'}점 · 상품 실사 {row.documentaryProductMediaCount}장 · candidate {row.candidateSnapshotStatus ?? '없음'} · proof {row.proofStatus ?? '없음'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  {row.customerReadinessBlockerCodes.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {row.customerReadinessBlockerCodes.map(code => (
                        <span key={code} className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-[10px] text-amber-800">{code}</span>
                      ))}
                    </div>
                  )}
                </div>
