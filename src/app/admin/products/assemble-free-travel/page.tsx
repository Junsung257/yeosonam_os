      ...
      {Array.from({ length: total }, (_, i) => (
        <div key={i} className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 ${
            ...
            i + 1 === current ? 'bg-blue-600 border-blue-600 text-white' :
            'bg-white border-admin-border-strong text-admin-muted-2'
          }`}>{i + 1 < current ? '✓' : i + 1}</div>
...
export default function AssembleFreeTravelPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold text-admin-text mb-1">반자유여행 조립기</h1>
      <p className="text-sm text-admin-muted mb-6">MRT 호텔 + 투어를 조합해 반자유여행 상품을 조립합니다. CS 필터 (평점 4.5↑ + 리뷰 100↑) 강제.</p>
              ...
              {hotels.map((h, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={!ok}
                    className={`w-full text-left p-3 rounded-admin-md border transition-all flex items-center gap-3 ${
                      sel ? 'border-blue-500 bg-blue-50' : ok ? 'border-admin-border-mid hover:border-blue-300' : 'border-admin-border opacity-50 cursor-not-allowed'
                    }`}
                  >
                    {h.imageUrl && <img src={h.imageUrl} alt={h.name} className="w-14 h-14 rounded-lg object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="text-sm font-semibold text-admin-text leading-tight">{h.name}</p>
                        <CsBadge ok={ok} />
              ...
              {activities.map((a, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-admin-md border transition-all flex items-center gap-3 ${
                      sel ? 'border-blue-500 bg-blue-50' : ok && selectedActs.size < 5 ? 'border-admin-border-mid hover:border-blue-300' : 'border-admin-border opacity-50 cursor-not-allowed'
                    }`}
                  >
                    {a.imageUrl && <img src={a.imageUrl} alt={a.name} className="w-12 h-12 rounded-lg object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="text-sm font-semibold text-admin-text leading-tight line-clamp-1">{a.name}</p>
                        <CsBadge ok={ok} />
                        {sel && <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded-full">선택됨</span>}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 text-xs text-admin-muted">
          ...
          <div className="bg-admin-surface rounded-admin-md border border-admin-border-mid shadow-admin-xs p-5 shadow-admin-xs">
            ...
            <div className="bg-admin-bg rounded-lg p-4 space-y-1.5 text-sm mb-4">
              <div className="flex justify-between">
                <span className="text-admin-muted">호텔 ({nights}박)</span>
                <span className="font-medium">{hotelPrice.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between">
                <span className="text-admin-muted">투어/액티비티 ({selectedActs.size}건)</span>
                <span className="font-medium">{actsPrice.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between text-admin-muted-2 text-xs">
                <span>기준: 성인 {adults}명</span>
              </div>
              <div className="border-t border-admin-border-mid pt-1.5 flex justify-between">
                <span className="text-admin-muted">합산 원가</span>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <input
                ...
                className="w-24 border border-admin-border-strong rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500"
              />
            ...
            <div className="border-t border-admin-border pt-3 space-y-1 text-xs text-admin-muted">
      ...
      {/* ── Step 4: 완료 ── */}
      {step === 4 && registered && (
        <div className="bg-green-50 border border-green-200 rounded-admin-md p-6 text-center">
