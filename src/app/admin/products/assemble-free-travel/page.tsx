    ...
    <div className="flex items-center gap-2 mb-6">
      {Array.from({ length: total }, (_, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          {i < total - 1 && <div className={`w-8 h-0.5 ${i + 1 < current ? 'bg-blue-600' : 'bg-slate-200'}`} />}
        </div>
      ))}
    </div>
  );
...
export default function AssembleFreeTravelPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold text-slate-900 mb-1">반자유여행 조립기</h1>
      <p className="text-sm text-slate-500 mb-6">MRT 호텔 + 투어를 조합해 반자유여행 상품을 조립합니다. CS 필터 (평점 4.5↑ + 리뷰 100↑) 강제.</p>
      ...
      {/* ── Step 1: 입력 ── */}
      {step === 1 && (
        <div className="bg-white rounded-xl border border-slate-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-6 shadow-sm space-y-4">
          <h2 className="text-lg font-semibold text-slate-800">1단계: 여행 정보 입력</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-slate-600 mb-1">목적지 도시</label>
              <input
                value={city}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">출발일</label>
              <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">귀국일</label>
              <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">성인</label>
              <input type="number" min={1} max={20} value={adults} onChange={e => setAdults(parseInt(e.target.value, 10))}
                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">아동</label>
      ...
      {step === 2 && (
        <div className="space-y-5">
          {/* 호텔 */}
          <div className="bg-white rounded-xl border border-slate-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-semibold text-slate-800">호텔 선택 (1개)</h2>
              <span className="text-xs text-slate-500">{hotels.length}건</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border transition-all flex items-center gap-3 ${
                      sel ? 'border-blue-500 bg-blue-50' : ok ? 'border-slate-200 hover:border-blue-300' : 'border-slate-100 opacity-50 cursor-not-allowed'
                    }`}
                  >
                    {h.imageUrl && <img src={h.imageUrl} alt={h.name} className="w-14 h-14 rounded-lg object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="text-sm font-semibold text-slate-900 leading-tight">{h.name}</p>
                        <CsBadge ok={ok} />
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-semibold text-slate-800">액티비티 선택 (최대 5개)</h2>
              <span className="text-xs text-slate-500">{selectedActs.size}/5 선택됨</span>
            </div>
              ...
              {activities.map((a, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border transition-all flex items-center gap-3 ${
                      sel ? 'border-blue-500 bg-blue-50' : ok && selectedActs.size < 5 ? 'border-slate-200 hover:border-blue-300' : 'border-slate-100 opacity-50 cursor-not-allowed'
                    }`}
                  >
                    {a.imageUrl && <img src={a.imageUrl} alt={a.name} className="w-12 h-12 rounded-lg object-cover shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="text-sm font-semibold text-slate-900 leading-tight line-clamp-1">{a.name}</p>
                        <CsBadge ok={ok} />
                        {sel && <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded-full">선택됨</span>}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500">
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="flex-1 py-2.5 border border-slate-300 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-50">
              이전
            </button>
            <button
      ...
      {step === 3 && selectedHotel && (
        <div className="space-y-4">
          {/* 선택 요약 */}
          <div className="bg-white rounded-xl border border-slate-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-800 mb-3">3단계: 가격 설정 + 등록</h2>

            <div className="bg-slate-50 rounded-lg p-4 space-y-1.5 text-sm mb-4">
              <div className="flex justify-between">
                <span className="text-slate-600">호텔 ({nights}박)</span>
                <span className="font-medium">{hotelPrice.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-600">투어/액티비티 ({selectedActs.size}건)</span>
              ...
              <div className="border-t border-slate-200 pt-1.5 flex justify-between">
                <span className="text-slate-600">합산 원가</span>
                <span className="font-semibold">{basePrice.toLocaleString()}원</span>
              </div>
            </div>

            <div className="flex items-center gap-3 mb-4">
              <label className="text-sm font-medium text-slate-700 shrink-0">마진율 (%)</label>
              <input
                type="number"
                min={0}
                max={50}
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(2)} className="flex-1 py-2.5 border border-slate-300 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-50">
              이전
            </button>
            <button
      ...
      {step === 4 && registered && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
          ...
          <p className="text-sm text-slate-600 mb-4">{registered.title}</p>
