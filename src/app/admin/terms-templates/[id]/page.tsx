...
export default function TermsTemplateEditPage(props: { params: Promise<Promise<{ id: string }> | { id: string }> }) {
        ...
        {tpl.notices.map((notice, idx) => (
          <div key={idx} className="border border-admin-border-mid rounded p-3 space-y-2 bg-admin-bg">
            <div className="grid grid-cols-3 gap-2">
              ...
              <select
                aria-label={`Notice ${idx + 1} severity`}
            ...
            <div className="flex items-center gap-3 text-xs">
              ...
              {ALL_SURFACES.map(s => (
                <label key={s} className="flex items-center gap-1">
                  <input
                    aria-label={`Notice ${idx + 1} surface ${s}`}
                    type="checkbox"
