    ...
    <div className="space-y-4">
      ...
      <div className="bg-admin-surface rounded-admin-md border border-admin-border-mid shadow-admin-xs overflow-hidden">
        <div className="px-3 py-2 border-b border-admin-border-mid flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <select aria-label="광고 초안 생성 상품 선택" value={selectedPkg?.id || ''} onChange={e => {
        ...
        <div className="max-h-72 overflow-y-auto">
          <table className="w-full">
            <thead className="sticky top-0 bg-admin-bg border-y border-admin-border-mid">
              <tr>
                <th className="w-8 py-1.5 px-2"><input type="checkbox" aria-label="Draft 광고 플랜 전체 선택" onChange={e => {
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(k => (
                <tr key={k.id} className="border-b border-admin-border hover:bg-admin-bg group">
                  <td className="py-1.5 px-2"><input type="checkbox" aria-label={`${k.keyword} 키워드 선택`} checked={selectedIds.has(k.id)} onChange={e => {
                  ...
                  <td className="text-admin-xs text-admin-text-2 py-1.5 px-2 text-right">
                    {editingBid === k.id ? (
                      <input type="number" aria-label={`${k.keyword} 입찰가 수정`} value={editBidValue} onChange={e => setEditBidValue(e.target.value)}
      ...
      {extractorOpen && (
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === 'Escape') setExtractorOpen(false);
          }}
        >
          ...
          >
            <div className="bg-white border-b border-admin-border-mid px-5 py-3 flex items-center justify-between flex-shrink-0">
              ...
              <button type="button" aria-label="키워드 추출기 닫기" onClick={() => setExtractorOpen(false)} className="p-1.5 text-admin-muted-2 hover:text-admin-muted"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
            </div>
        ...
        >
          ...
          >
            <div className="bg-white border-b border-admin-border-mid px-5 py-3 flex items-center justify-between flex-shrink-0">
              ...
              <button type="button" aria-label="AI 입찰 최적화 닫기" onClick={() => setOptimizerOpen(false)} className="p-1.5 text-admin-muted-2 hover:text-admin-muted"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
            </div>
