...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    rejected: 'bg-admin-surface-2 text-admin-muted',
  };
  const label: Record<string, string> = { approved: '노출', pending: '대기', rejected: '숨김' };
  return (
    <span className={`text-[11px] font-bold px-2 py-0.5 rounded ${map[status] || 'bg-admin-surface-2 text-admin-muted'}`}>
      {label[status] || status}
    </span>
  );
}
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-admin-text-2">⭐ 고객 후기 관리</h1>
      ...
      {/* 등록 폼 */}
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 bg-blue-50 border border-blue-200 rounded-admin-lg p-5 space-y-4">
          <h2 className="font-bold text-admin-text-2 text-base">📝 후기 직접 입력 (카카오 피드백 기반)</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-admin-border-mid rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand focus:outline-none"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-admin-border-mid rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand focus:outline-none resize-none"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-admin-border-mid rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand focus:outline-none resize-none font-mono"
            />
          </div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="button" onClick={() => { setShowForm(false); setForm(DEFAULT_FORM); }}
              className="px-4 py-2 bg-admin-surface-2 text-admin-muted text-sm rounded-lg hover:bg-slate-200">
              취소
            </button>
          </div>
        </form>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3].map(i => (
            <div key={i} className="h-24 bg-admin-surface-2 rounded-admin-md animate-pulse" />
          ))}
        </div>
      ) : reviews.length === 0 ? (
        ...
        <div className="space-y-3">
          {reviews.map(r => {
            ...
            return (
              <div key={r.id} className={`p-4 border rounded-admin-md transition-colors ${r.status === 'approved' ? 'bg-white border-admin-border-mid' : 'bg-admin-bg border-admin-border-mid opacity-60'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(n => (
                          <span key={n} className={`text-sm ${n <= r.overall_rating ? 'text-amber-400' : 'text-admin-border-mid'}`}>★</span>
                        ))}
                      </div>
                      ...
                      <span className="text-[11px] text-admin-muted-2">{maskedName}</span>
                      <span className="text-[11px] text-admin-muted-2">
                        {fmtDateISO(r.created_at)}
                      </span>
                    </div>
                    {r.title && <p className="text-sm font-bold text-admin-text-2 mb-1">{r.title}</p>}
                    {r.review_text && (
                      <p className="text-sm text-admin-muted line-clamp-3 leading-relaxed">{r.review_text}</p>
                    )}
      ...
      {/* 토스트 */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-sm px-5 py-2.5 rounded-full shadow-admin-lg z-50">
          {toast}
        </div>
      )}
    </div>
