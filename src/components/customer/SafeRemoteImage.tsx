...
export function SafeCoverImg({
  ...
  const srcSet = buildBlogImageSrcSet(src);
...
export function SafeMagazineThumb({ url, title, placeholderClassName }: MagazineProps) {
  ...
  const srcSet = buildBlogImageSrcSet(url);
  ...
  return (
    <div className="aspect-[16/9] bg-slate-100 overflow-hidden">
      <img
        src={displaySrc.trim()}
        srcSet={srcSet}
        sizes={srcSet ? '(max-width: 640px) 100vw, 33vw' : undefined}
        alt={title}
        width={1600}
        height={900}
