...
interface ClassificationResponse {
  ...
  summary: { transactionTotal: number; itemTotal: number; reviewTransactionTotal: number; companyReviewTransactionTotal: number; travelReviewTransactionTotal: number; review: number; batchEligibleReview: number; resolved: number; manual: number; missingReceipt: number };
  error?: string;
}
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-admin-md border border-amber-200 bg-amber-50 p-4"><p className="text-xs text-amber-700">검토 필요</p><p className="mt-1 text-xl font-bold text-amber-900">{data?.summary.review ?? '...' }항목</p><p className="mt-1 text-[10px] leading-4 text-amber-800">회사 미분류 {data?.summary.companyReviewTransactionTotal ?? '...'}건 · 여행 가능성 {data?.summary.travelReviewTransactionTotal ?? '...'}건<span className="block">바로 분류 {data?.summary.batchEligibleReview ?? '...'}항목 · 개별 확인 {(data?.summary.review ?? 0) - (data?.summary.batchEligibleReview ?? 0)}항목</span></p></div>
