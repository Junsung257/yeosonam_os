...
interface ClassificationResponse {
  ...
  summary: { transactionTotal: number; itemTotal: number; review: number; batchEligibleReview: number; resolved: number; manual: number; missingReceipt: number };
  error?: string;
}
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-admin-md border border-amber-200 bg-amber-50 p-4"><p className="text-xs text-amber-700">미분류·개별 확인</p><p className="mt-1 text-xl font-bold text-amber-900">{data?.summary.review ?? '...' }건</p><p className="mt-1 text-[10px] text-amber-800">바로 분류 가능 {data?.summary.batchEligibleReview ?? '...'}건 · 예약 가능성 {(data?.summary.review ?? 0) - (data?.summary.batchEligibleReview ?? 0)}건</p></div>
