...
function BookingDrawer({
  ...
  useEffect(() => {
    nestedDialogOpenRef.current = Boolean(decision || splitTransactionId || feeSuggestionTransactionId);
  }, [decision, splitTransactionId, feeSuggestionTransactionId]);
  useEffect(() => {
    const previous = document.activeElement as HTMLElement | null;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex justify-end">
      <button type="button" tabIndex={-1} className="absolute inset-0 bg-slate-950/35" onClick={onClose} aria-label="상세 닫기" />
      <aside ref={drawerRef} role="dialog" aria-modal="true" aria-label="예약 정산 상세" tabIndex={-1} className="relative h-full w-full max-w-3xl overflow-y-auto border-l border-slate-200 bg-white shadow-2xl">
        <header className="sticky top-0 z-20 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4">
          ...
          <div className="flex items-center gap-2">{nextBookingId ? <button type="button" onClick={() => onSelectBooking(nextBookingId)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700">다음 미검토 예약</button> : null}<button ref={closeButtonRef} type="button" onClick={onClose} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="닫기"><X className="h-5 w-5" /></button></div>
        </header>
