          ...
          <div className="flex min-h-80 flex-col justify-between p-5 sm:p-7">
            {currentTask ? <>
              <div>
                ...
                {workday.scope.futurePendingBookingCount > 0 || workday.scope.priorPendingBookingCount > 0 || workday.scope.undatedPendingBookingCount > 0 || workday.scope.otherMonthTravelReviewCount > 0 ? <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-3 text-xs text-sky-950"><strong>{workday.closeMonth} 마감과 분리된 업무</strong><span className="mt-1 block">향후 출발 {workday.scope.futurePendingBookingCount}건 · 과거 누락 {workday.scope.priorPendingBookingCount}건 · 출발일 미정 {workday.scope.undatedPendingBookingCount}건 · 다른 출발 월 거래 확인 {workday.scope.otherMonthTravelReviewCount}건은 이번 월마감을 차단하지 않습니다.</span></div> : null}
