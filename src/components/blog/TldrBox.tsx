...
export default function TldrBox({ title = '핵심 요약', items }: Props) {
  ...
  return (
    <aside
      className="not-prose mb-10 rounded-2xl border border-[#DBEAFE] bg-gradient-to-br from-[#EBF3FE]/70 to-[#F0F7FF]/40 p-5 md:p-6"
      aria-label={title}
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold tracking-tight text-[#191F28]">{title}</h2>
      </div>
      <ul className="space-y-2 text-[15px] leading-relaxed text-gray-700 md:text-[15.5px]">
        {clean.map((item, i) => (
          <li key={i} className="flex gap-2">
            <span className="mt-[9px] h-1.5 w-1.5 shrink-0 rounded-full bg-[#3182F6]" aria-hidden="true" />
            <span>{item}</span>
          </li>
        ))}
      </ul>
