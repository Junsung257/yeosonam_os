---
name: register
description: 통합 상품 등록 — 원문 + 랜드사·마진 입력으로 파싱, 등록, 감사, 승인 후보, 결과 리포트까지 수행한다. 사용자 명시 호출만 허용한다.
...
> 모델 이름을 스킬에 고정하지 않는다. 현재 AI Ops 라우터의 고정확도 경로와 검증 게이트를 사용한다.
...
| [`references/region-and-ir-pipe.md`](references/region-and-ir-pipe.md) | IR 파이프 엔진 결정과 신규 지역 부트스트랩 | ~74 |
...
항상 `docs/product-registration-current-ssot.md`를 먼저 확인한다. 다음 자료는 입력에 해당할 때만 읽는다.

1. 반복 오류가 의심되면 `db/error-registry.md`의 관련 항목
2. 신규·미매칭 관광지가 있으면 `.agents/skills/source-command-manage-attractions/SKILL.md`
3. 표준 문구·REMARK가 있으면 `docs/product-registration-v3-standard-language.md`

## 🌱 관광지 처리 정책 (STRICT SSOT)

> **권위 문서**: [`docs/product-registration-v3-standard-language.md`](../../../docs/product-registration-v3-standard-language.md), [`.agents/skills/source-command-manage-attractions/SKILL.md`](../source-command-manage-attractions/SKILL.md)
...
IR 파이프의 현재 엔진 선택은 [`references/region-and-ir-pipe.md`](references/region-and-ir-pipe.md).
