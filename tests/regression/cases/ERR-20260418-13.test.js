/**
 * @case ERR-20260418-13
 * @summary A4 flight labels must stay compact by normalizing airline codes
 * before rendering headers and flight labels.
 */
...
test('ERR-20260418-13: render contract normalizes airline labels centrally', () => {
  const source = read('src/lib/render-contract.ts');

  assert.match(source, /ERR-20260418-13\/17/);
  assert.match(source, /const AIRLINE_MAP: Record<string, string>/);
  ...
  assert.match(source, /parenMatch/);
});

test('ERR-20260418-13: unit tests cover code, numeric flight, parenthesized, and pipe forms', () => {
  ...
  assert.match(testSource, /BX \|/);
  assert.match(testSource, /7C123/);
  assert.match(testSource, /5J789/);
