...
type InngestRuntimeEvidence = Omit<
  InngestIntrospectionEvidence,
  'inngestEndpointReachable' | 'error'
>;
...
function readInngestRuntimeEvidence(path: string): InngestRuntimeEvidence {
  const payload = JSON.parse(readFileSync(resolve(path), 'utf8')) as {
    automation?: {
      mode?: unknown;
      hasEventKey?: unknown;
      hasSigningKey?: unknown;
      functionCount?: unknown;
      minimumFunctionCount?: unknown;
    };
  };
  const automation = payload.automation;
  if (!automation) throw new Error('blog_data_readiness_inngest_evidence_missing');
  return {
    mode: typeof automation.mode === 'string' ? automation.mode : null,
    hasEventKey: automation.hasEventKey === true,
    hasSigningKey: automation.hasSigningKey === true,
    functionCount: typeof automation.functionCount === 'number'
      && Number.isInteger(automation.functionCount)
      ? automation.functionCount
      : null,
    minimumFunctionCount: typeof automation.minimumFunctionCount === 'number'
      && Number.isInteger(automation.minimumFunctionCount)
      ? automation.minimumFunctionCount
      : 5,
  };
}

async function inspectInngestEndpoint(
  base: string,
  runtimeEvidence: InngestRuntimeEvidence,
): Promise<InngestIntrospectionEvidence> {
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      let signedCloudEndpoint = false;
      if (contentType.includes('application/json')) {
        try {
          const payload = JSON.parse(raw) as { message?: unknown };
          signedCloudEndpoint = payload.message === 'Unauthorized';
        } catch {
          signedCloudEndpoint = false;
        }
      }
      if (signedCloudEndpoint) {
        // Inngest SDK v4 intentionally rejects unsigned GET introspection in
        // cloud mode. This exact JSON 401 proves the serve endpoint is live and
        // enforcing its signing key; safe details come from the authenticated
        // blog-data-readiness response generated by the same deployment.
        return {
...
async function main(): Promise<void> {
  ...
  const dataReadinessPath = argument('data-readiness');
  if (!dataReadinessPath) throw new Error('blog_data_readiness_evidence_required');
  const automation = await inspectInngestEndpoint(
    base,
    readInngestRuntimeEvidence(dataReadinessPath),
  );
