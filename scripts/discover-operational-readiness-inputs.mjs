...
if (missingConnection.length === 0) {
  ...
  const isReadinessCard = (row) => String(row?.title || '').startsWith('[READINESS]');
  const cardRow = cardRows.find((row) => row?.id && !isReadinessCard(row))
    || cardRows.find((row) => row?.id)
    || null;
  ...
  report.discovered = {
    ...
    MARKETING_CHECK_VARIANT_GROUP_ID: currentOrDiscovered(
      ...
      'No card_news row with a non-empty variant_group_id was found. Create or keep one real variant group before full dynamic marketing page verification.',
    ),
    SUPABASE_PROJECT_REF: currentOrDiscovered(
  ...
  for (const key of [
    ...
    report.discovered[key] = currentOrDiscovered(
      key,
      '',
      'missing',
      `Cannot query Supabase until ${missingConnection.join(', ')} is available.`,
    );
  }
}
...
report.missingReasons = Object.fromEntries(
  Object.values(report.discovered)
