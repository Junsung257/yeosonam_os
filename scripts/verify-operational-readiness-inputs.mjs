...
const groups = [
  {
    ...
    severity: 'blocked',
    keys: ['OPEN_CHECK_PACKAGE_ID', 'OPEN_CHECK_REF_CODE', 'OPEN_CHECK_BLOG_SLUG'],
    notes: 'Required to verify package detail, blog detail, and referral-link flows against real data.',
  },
  {
...
function writeApplyScript(path, checks) {
  ...
  const lines = [
    ...
    '# Example: OPEN_CHECK_PACKAGE_ID=... OPEN_CHECK_REF_CODE=... OPEN_CHECK_BLOG_SLUG=... bash .tmp/operational-readiness-apply-inputs.sh',
    '',
...
function writeNodeApplyScript(path, checks) {
  ...
  const lines = [
    ...
    ' * Or pass --env-file=<filled-template> / OPERATIONAL_INPUTS_ENV_FILE=<filled-template>.',
    ' * Set OPERATIONAL_APPLY_DRY_RUN=1 to print the commands without applying changes.',
    ` * Example: OPEN_CHECK_PACKAGE_ID=... OPEN_CHECK_REF_CODE=... OPEN_CHECK_BLOG_SLUG=... node ${nodeScriptComment(path)}`,
    ' */',
    '',
    ...
    '    const result = spawnSync(candidate, args, { stdio: "inherit", windowsHide: true });',
    '    last = result;',
...
function writeNodeVercelScript(path, checks) {
  ...
  const lines = [
    ...
    '  let last;',
    '  for (const candidate of attempts) {',
    '    const result = spawnSync(candidate, args, { stdio: options.stdio || "inherit", windowsHide: true });',
    '    last = result;',
    '    if (!result.error || result.error.code !== "ENOENT") {',
