...
try {
  ...
  const manifest = JSON.parse(readFileSync(
    resolve('supabase/release-manifests/blog-editorial-harness-v5-20260830.json'),
    'utf8',
  ...
  process.stderr.write(`blog V5 Supabase dry-run verification failed: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 1;
}
