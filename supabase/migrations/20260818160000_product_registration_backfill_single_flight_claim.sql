-- Prevent overlapping cron/manual backfill invocations from waiting on the
-- same package rows until the database statement timeout. A concurrent caller
-- exits cleanly; the next scheduled tick will claim the remaining work.
create or replace function public.claim_product_registration_legacy_backfill(
  p_limit integer default 10,
  p_engine_version text default 'product-registration-v6-workflow-26'
)
returns jsonb
language plpgsql
security definer
...
begin
  if v_engine_version is null then raise exception 'REGISTRATION_BACKFILL_ENGINE_VERSION_REQUIRED'; end if;
  if not pg_try_advisory_xact_lock(hashtext('product-registration-v6-legacy-backfill-claim')) then
    return '[]'::jsonb;
  end if;
  ...
  with candidates as (
    ...
    where p.catalog_product_id is not null
      ...
      and (
        b.id is null
        or (b.status = 'failed' and b.engine_version = v_engine_version and b.attempt_count < 3 and b.updated_at < now() - interval '30 minutes')
        or (b.status in ('failed', 'blocked') and b.engine_version is distinct from v_engine_version)
        or (b.status in ('reserved', 'started') and b.engine_version is distinct from v_engine_version and b.updated_at < now() - interval '30 minutes')
      )
    order by
    ...
    select tenant_id, catalog_product_id, package_id, 'reserved', 1, 1, v_engine_version, null, now()
    from candidates
    on conflict (tenant_id, catalog_product_id) do update
      set status = 'reserved',
          attempt_count = case when internal_product_registration.legacy_backfill_jobs.engine_version is distinct from v_engine_version then 1 else internal_product_registration.legacy_backfill_jobs.attempt_count + 1 end,
      ...
      where (
        ...
        internal_product_registration.legacy_backfill_jobs.status in ('reserved', 'started')
        and internal_product_registration.legacy_backfill_jobs.engine_version is distinct from v_engine_version
        and internal_product_registration.legacy_backfill_jobs.updated_at < now() - interval '30 minutes'
      )
...
revoke all on function public.claim_product_registration_legacy_backfill(integer, text) from public, anon, authenticated;
grant execute on function public.claim_product_registration_legacy_backfill(integer, text) to service_role;
revoke all on function public.claim_product_registration_legacy_backfill(integer) from public, anon, authenticated, service_role;
