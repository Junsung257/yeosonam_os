-- Retry ceilings are scoped to one deployed Kernel/workflow version. A newer
-- engine may re-evaluate only prior infrastructure failures, while evidence-
-- based business blocks remain terminal and never loop after deployment.

alter table internal_product_registration.legacy_backfill_jobs
  add column if not exists engine_version text,
  add column if not exists total_attempt_count integer not null default 1
    check (total_attempt_count >= 1);

create index if not exists idx_registration_legacy_backfill_engine_retry
  on internal_product_registration.legacy_backfill_jobs(status, engine_version, updated_at)
  where status in ('failed', 'blocked');

create or replace function public.claim_product_registration_legacy_backfill(
  p_limit integer default 10,
  p_engine_version text default 'product-registration-v6-workflow-2'
)
returns jsonb
language plpgsql
security definer
  ...
  with candidates as (
    ...
    where p.catalog_product_id is not null
      ...
      and (
        ...
        or (
          ...
          and b.last_error like 'WORKFLOW_FAILED:%'
        )
      )
    order by
    ...
    on conflict (tenant_id, catalog_product_id) do update
      ...
      where (
        ...
        and internal_product_registration.legacy_backfill_jobs.last_error like 'WORKFLOW_FAILED:%'
      )
...
-- Prevent stale callers from bypassing engine-version-scoped retry semantics.
