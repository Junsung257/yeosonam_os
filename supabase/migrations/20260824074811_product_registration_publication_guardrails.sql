...
-- V5 convergence rows originally carried only package lineage. The admin truth
-- projection is catalog-product scoped, so retain that immutable identity on
-- both historical and future convergence evidence before creating the view.
alter table public.product_registration_v5_cache_convergence_runs
  add column if not exists catalog_product_id uuid
    references internal_product_registration.catalog_products(id) on delete restrict;

update public.product_registration_v5_cache_convergence_runs convergence
set catalog_product_id = package.catalog_product_id
from public.travel_packages package
where convergence.catalog_product_id is null
  and package.id = convergence.package_id
  and package.catalog_product_id is not null;

create index if not exists idx_pr_v5_convergence_catalog_product
  on public.product_registration_v5_cache_convergence_runs(
    tenant_id, catalog_product_id, snapshot_id
  );
