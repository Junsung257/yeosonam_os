-- Blog DeepSeek V4 auto-repair budget
--
-- A quality score is an instruction to repair, not an automatic quarantine
-- reason. This additive migration raises the durable model-call budget from
-- three to five so a candidate can receive two additional DeepSeek Pro repair
-- passes. It does not relax factual, claim, risk, duplicate, or publication
-- gates. Apply only after the matching application release is deployed.
--
-- Dry-run: inspect the affected constraints/function definition before apply.
-- Rollback (only if no attempt_number > 3 exists):
...
--   -- 20260816015102_blog_ai_budget_deepseek_only.sql.
...
alter table public.blog_generation_runs
  drop constraint if exists blog_generation_runs_attempt_count_check,
  drop constraint if exists blog_generation_runs_attempt_count_v3_check,
  drop constraint if exists blog_generation_runs_attempt_count_v5_check;
alter table public.blog_generation_runs
  add constraint blog_generation_runs_attempt_count_v5_check
    check (attempt_count between 0 and 5);

alter table public.blog_generation_attempts
  drop constraint if exists blog_generation_attempts_attempt_number_check,
  drop constraint if exists blog_generation_attempts_attempt_number_v3_check,
  drop constraint if exists blog_generation_attempts_attempt_number_v5_check;
alter table public.blog_generation_attempts
  add constraint blog_generation_attempts_attempt_number_v5_check
    check (attempt_number between 1 and 5);

alter table public.blog_ai_budget_reservations
  drop constraint if exists blog_ai_budget_reservations_attempt_number_check,
  drop constraint if exists blog_ai_budget_reservations_attempt_number_v3_check,
  drop constraint if exists blog_ai_budget_reservations_attempt_number_v5_check;
alter table public.blog_ai_budget_reservations
  add constraint blog_ai_budget_reservations_attempt_number_v5_check
    check (attempt_number between 1 and 5);
