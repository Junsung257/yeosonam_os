...
set
  intents = case
    when 'family_budget' = any(intents) then intents
    else array_append(intents, 'family_budget')
  end,
  reviewed_by = 'codex_family_budget_meal_scope_correction',
  reviewed_at = now(),
  review_note = 'First-party checked-date breakfast and beverage menu. Use exact meal or snack samples only and never infer a destination average.',
  updated_at = now()
