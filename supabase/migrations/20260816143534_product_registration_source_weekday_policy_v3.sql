-- Product Registration: new jobs use weekday-aware yearless-date policy v3.
-- Existing jobs remain pinned to their original policy version, while the DB
-- future-date guard protects writes from both v2 and v3 jobs.

alter table public.upload_jobs
  alter column v6_date_policy_version set default 'source-departure-date-policy-3';

comment on column public.upload_jobs.v6_date_policy_version is
  'Immutable date-resolution policy pinned per job. V3 rejects stale weekday rollover; legacy rows are never silently upgraded.';
