-- ============================================================
-- 여소남 OS: 마케팅 RPC 생성 및 자동화 파이프라인 SQL 마이그레이션
-- 마이그레이션: 20260524020000
--
-- 목적:
--   1. refresh_attribution_summary() : attribution_touch_events/chains 기반 요약 집계
--   2. recompute_rfm_scores() : bookings 기반 customer_rfm UPSERT
--   3. auto_heal_content_gaps() : 콘텐츠 공백 탐지 후 blog_topic_queue 등록
--   4. auto_finalize_ab_experiments() : 유의미한 A/B 테스트 자동 종료 처리
--   5. generate_predictive_insights() : keyword_trend_snapshots 기반 인사이트 생성
-- ============================================================

BEGIN;

-- ============================================================
-- 0. blog_topic_queue source CHECK에 'auto_heal' 추가
-- ============================================================
...
-- ============================================================
-- attribution_chains.touchpoints(JSONB)를 기준으로 5가지 모델을 계산
-- attribution_summary를 갱신(최근 30일 데이터 기준)
-- ============================================================
...
BEGIN
  -- attribution_summary 초기화 (재집계를 위한 정리)
  ...
  -- 모델별 누적값 병합(channel + creative_id + campaign_id 단위 SUM)
  WITH merged AS (
    SELECT
      channel,
      creative_id,
...
-- ============================================================
-- bookings 데이터 기반 RFM(Recency, Frequency, Monetary) 계산
-- 계산 결과를 customer_rfm에 UPSERT
-- Quintile 기준으로 1~5 점수 부여.
-- ============================================================
