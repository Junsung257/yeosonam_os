...
WITH stats AS (
  SELECT
    tenant_id,
    AVG(final_payout) AS avg_amount,
    STDDEV(final_payout) AS stddev_amount,
    COUNT(*) AS n
  FROM settlements
...
SELECT
  ...
  s.tenant_id,
  s.final_payout AS amount,
  st.avg_amount,
  st.stddev_amount,
  CASE
    WHEN st.stddev_amount > 0 AND ABS(s.final_payout - st.avg_amount) > 3 * st.stddev_amount THEN 'CRITICAL'
    WHEN st.stddev_amount > 0 AND ABS(s.final_payout - st.avg_amount) > 2 * st.stddev_amount THEN 'WARNING'
    ELSE 'NORMAL'
  END AS anomaly_level,
  ROUND(ABS(s.final_payout - st.avg_amount) / NULLIF(st.stddev_amount, 0), 2) AS z_score,
  NOW() AS detected_at
FROM settlements s
...
WHERE s.created_at >= NOW() - INTERVAL '7 days'
  ...
  AND (
    (st.stddev_amount > 0 AND ABS(s.final_payout - st.avg_amount) > 2 * st.stddev_amount)
    OR s.final_payout <= 0
  )
...
ORDER BY ABS((d.cnt - b.avg_daily) / NULLIF(b.stddev_daily, 0)) DESC;
