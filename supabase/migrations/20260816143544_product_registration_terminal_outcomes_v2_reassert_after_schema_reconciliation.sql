-- Every registration input must end in an explicit business outcome. Parser
-- failures and unsupported sources are quarantined; they are not counted as
-- product-quality review work or safe-publication failures.
...
declare
  ...
  v_terminal_outcome text := p_payload->>'compatibility_outcome';
...
begin
  ...
  if v_terminal_outcome not in (
    ...
    'quarantined_unsupported_or_corrupt',
    'quarantined_system_failure'
  ) then raise exception 'V6_TERMINAL_OUTCOME_INVALID'; end if;
  ...
  if v_analysis_outcome = 'blocked'
    and v_terminal_outcome in (
      'published_verified', 'published_degraded',
      'ready_verified_not_published', 'ready_degraded_not_published'
    ) then
    raise exception 'V6_TERMINAL_ANALYSIS_COMPATIBILITY_MISMATCH';
  end if;
  if v_analysis_outcome = 'verified'
    and v_terminal_outcome not in ('published_verified', 'ready_verified_not_published') then
    raise exception 'V6_TERMINAL_ANALYSIS_COMPATIBILITY_MISMATCH';
  end if;
  if v_analysis_outcome = 'degraded'
    and v_terminal_outcome not in ('published_degraded', 'ready_degraded_not_published') then
    raise exception 'V6_TERMINAL_ANALYSIS_COMPATIBILITY_MISMATCH';
  end if;
  if v_terminal_outcome in ('published_verified', 'published_degraded')
    and v_publication_state <> 'converged' then
    raise exception 'V6_PUBLISHED_TERMINAL_REQUIRES_CONVERGENCE';
  end if;
  if v_terminal_outcome in ('ready_verified_not_published', 'ready_degraded_not_published')
    and v_publication_state not in ('frozen', 'blocked', 'proof_passed', 'convergence_failed') then
    raise exception 'V6_READY_TERMINAL_PUBLICATION_STATE_INVALID';
  end if;
  if v_terminal_outcome not in (
      'published_verified', 'published_degraded',
      'ready_verified_not_published', 'ready_degraded_not_published'
    )
    and v_publication_state not in ('not_requested', 'blocked') then
    raise exception 'V6_NON_PUBLISHED_TERMINAL_PUBLICATION_FORBIDDEN';
  end if;
  ...
  set v6_workflow_run_id = coalesce(v6_workflow_run_id, v_workflow_run_id),
      ...
      status = case
        when v_terminal_outcome in (
          'discarded_source_incomplete', 'discarded_non_travel',
          'discarded_duplicate_or_consolidated', 'archived_all_departures_past'
        ) then 'done'
        when v_terminal_outcome like 'quarantined_%' then 'failed'
      ...
      v4_stage = case
        ...
        when v_terminal_outcome like 'quarantined_%' then 'quarantined'
  ...
  if not found then
    select * into v_job from public.upload_jobs
    ...
    if not found
      ...
      or v_job.v6_outcome is distinct from v_terminal_outcome then
  ...
  return jsonb_build_object(
    ...
    'terminal_outcome', v_job.v6_outcome,
