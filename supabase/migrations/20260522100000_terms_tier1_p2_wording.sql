-- P2: Tier 1 플랫폼 기본약관 문구·surface 정리 (법무 검토 전 기술 초안)
--   - RESERVATION: 실비 우선 조항을 표 상단으로 이동 (모바일 P0 억제와 병행, A4 전문 참조용)
--   - AUTO_TICKETING: A4 surface 추가 (인쇄·포스터 전문에 자동발권 조항 포함)
-- ON CONFLICT (name) DO UPDATE → 멱등

UPDATE public.terms_templates
SET
  notices = $notices$[
    {
      "type": "AUTO_TICKETING",
  ...
  notes = 'P2 v3: RESERVATION 실비 우선 조항 상단 이동·AUTO_TICKETING A4 surface. 법무 최종 검토 대기.',
  version = version + 1,
  updated_at = now()
WHERE name = '여소남 플랫폼 기본약관 v1';
