...
CREATE POLICY customers_owner_select ON public.customers
  ...
  USING (auth.jwt() ->> 'role' = 'authenticated' AND (id::text = (auth.jwt() ->> 'sub')));
...
CREATE POLICY bookings_customer_select ON public.bookings
  AS PERMISSIVE FOR SELECT
  USING (auth.jwt() ->> 'role' = 'authenticated' AND (lead_customer_id::text = (auth.jwt() ->> 'sub')));
...
CREATE POLICY customer_notes_owner_select ON public.customer_notes
  ...
  USING (auth.jwt() ->> 'role' = 'authenticated' AND
         EXISTS (SELECT 1 FROM customers c WHERE c.id = customer_notes.customer_id AND c.id::text = (auth.jwt() ->> 'sub')));
...
CREATE POLICY booking_companions_customer_select ON public.booking_companions
  AS PERMISSIVE FOR SELECT
  USING (auth.jwt() ->> 'role' = 'authenticated' AND
         EXISTS (SELECT 1 FROM bookings b WHERE b.id = booking_companions.booking_id AND b.lead_customer_id::text = (auth.jwt() ->> 'sub')));
...
CREATE POLICY conversations_participant_select ON public.conversations
  AS PERMISSIVE FOR SELECT
  USING (auth.jwt() ->> 'role' = 'authenticated' AND
         (participant_1_id::text = (auth.jwt() ->> 'sub') OR participant_2_id::text = (auth.jwt() ->> 'sub')));
...
CREATE POLICY secure_chats_participant_select ON public.secure_chats
  ...
  USING (auth.jwt() ->> 'role' = 'authenticated' AND
         (sender_id::text = (auth.jwt() ->> 'sub') OR
          EXISTS (SELECT 1 FROM bookings b WHERE b.id = secure_chats.booking_id AND b.lead_customer_id::text = (auth.jwt() ->> 'sub'))));
