-- Reconcile the production attribution schema with the tracking contract.
-- The original 20260608153000 migration was skipped in production history,
...
ALTER TABLE public.ad_conversion_logs
  ...
  ADD COLUMN IF NOT EXISTS first_touch_ad_landing_mapping_id uuid
    REFERENCES public.ad_landing_mappings(id) ON DELETE SET NULL,
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ad_traffic_logs_gbraid
  ON public.ad_traffic_logs(gbraid)
  WHERE gbraid IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ad_traffic_logs_wbraid
  ON public.ad_traffic_logs(wbraid)
  WHERE wbraid IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ad_conversion_logs_paid_assisted
  ON public.ad_conversion_logs(paid_assisted_organic, created_at DESC)
  WHERE paid_assisted_organic = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ad_conversion_logs_first_touch_mapping
