-- Permit an explicitly seeded, draft-only staging canary to recover from a
-- failed provider/contract retry. Normal failed and human-review operations
-- remain terminal. The queue owner and draft-only checks are the complete
-- authorization boundary for this recovery path.
...
begin
  ...
  if v_operation.status = 'research_backlog' then
    ...
    where id = p_operation_id
    returning * into v_operation;
  elsif v_operation.status in ('human_review', 'failed')
