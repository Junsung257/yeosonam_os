-- Reclaim abandoned legacy backfill claims when a previous engine version
-- reserved/started a row but never completed its workflow bind. The durable
-- workflow remains the source of truth; only stale rows from an older engine
-- version are eligible, preventing a live current-version run from being
-- double-started.

create or replace function public.claim_product_registration_legacy_backfill(
  p_limit integer default 10,
  p_engine_version text default 'product-registration-v6-workflow-25'
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  with candidates as (
    ...
    order by
      case
        when b.id is not null and b.engine_version is distinct from v_engine_version then 2000
        when b.status = 'failed' then 1000
        else 0
      end desc,
      (
