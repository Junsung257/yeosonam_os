-- Enforce provider JSON mode for future editorial-judge calls and allow one
-- final structured retry for attempts whose original and legacy text-mode
-- retry were both billed but unparsable. This remains separate from article
-- generation and under the same atomic KST-day cost cap.
...
begin
  if p_requested_usd <= 0 or p_cap_usd <= 0
    ...
    or (p_call_kind in (
      ...
      'editorial_judge_retry',
      'editorial_judge_structured_retry'
    ) and p_stage <> 'editorial_judge') then
...
comment on constraint blog_ai_budget_reservations_call_kind_check
  ...
  'Separates article generation, editorial judging, and one structured recovery retry under a shared daily cap.';
