...
DO $$
DECLARE
  total_price_is_generated boolean;
  total_cost_is_generated boolean;
BEGIN
  SELECT attgenerated <> ''
  INTO total_price_is_generated
  FROM pg_attribute
  ...
  SELECT attgenerated <> ''
  INTO total_cost_is_generated
  FROM pg_attribute
  WHERE attrelid = 'public.bookings'::regclass
    AND attname = 'total_cost'
    AND NOT attisdropped;

  IF NOT COALESCE(total_price_is_generated, false) THEN
  ...
  IF NOT COALESCE(total_cost_is_generated, false) THEN
    UPDATE public.bookings
    SET
      total_cost =
